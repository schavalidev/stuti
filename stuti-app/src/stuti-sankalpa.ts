import { SK_DATA } from "./stuti-panchanga";
...
/* ============================================================
   ...
   ============================================================ */
export const STUTI_SANKALPA = (function () {
  const seg = (deva, iast, em) => ({ deva, iast, em: !!em });
  const SK = () => STUTI_SK || SK_DATA || SK_CONST || {};
  ...
  function identity(sk) {
    sk = sk || (STUTI_FLYLEAF ? STUTI_FLYLEAF.get() : { gotra: "", nama: "", gender: "male", karma: "parayana" });
    const S = SK();
      const P = STUTI_PITRU;
  ...
  function segs(o) {
    o = o || {};
    const PA = AKSHARA_PANCHANGA, S = SK();
    const date = o.date || new Date();
    const loc = o.loc || place();
    const pa = o.pa || PA.forDay(date, loc, { instant: true });
    const sk = o.sk || (STUTI_FLYLEAF ? STUTI_FLYLEAF.get() : { gotra: "", nama: "", gender: "male", karma: "parayana" });
    const upto = o.upto || "full";
    const samv = samvatsaraFor ? samvatsaraFor(date) : S.samvatsaraFor(date);
    ...
    const saura = o.saura != null ? o.saura : (typeof manaSys === "function" && manaSys() === "saura");

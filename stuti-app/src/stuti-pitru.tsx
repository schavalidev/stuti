...
function TarpanaWhy({ id, date, lang = "deva", onClose }) {
  const TP = STUTI_TARPANA, P = AKSHARA_PANCHANGA, L = STUTI_L;
  const w = usePiM(() => { try { return TP.why(id, date); } catch (e) { return null; } }, [id, +date]);
  if (!w) return null;
  const kalaName = (k) => piPick((TP.KALA[k] || {}).label, lang) || k;
  const tithiName = (ti) => { try { return piPick(STUTI_TITHIS.tithiName(ti), lang); } catch (e) { return String(ti + 1); } };
  return (
    <OverlayPortal>
              ...
              <div className="pit-src">
                ...
                {w.sources.map((s, i) => (
                  <a key={i} href={s.url} target="_blank" rel="noreferrer noopener" className="pit-src-row">
                    <span>{s.label}</span><Icon name="chev" size={14} />
                  </a>
                ))}
              </div>
            )}
...
function PitruDaysBody({ lang = "deva" }) {
  const TP = STUTI_TARPANA, V = STUTI_VRATA, L = STUTI_L;
  ...
  const mine = usePiM(() => {
    ...
    try {
      ...
      return (STUTI_TITHIS.list() || []).filter((r) => r.kind === "shraddha" && set.indexOf(r.ti) !== -1);
    } catch (e) { return []; }
  }, [year]);
          ...
          {ahead.map((u) => {
            ...
            return (
              <div className="pit-row" key={u.v.id + u.date.getTime()}>
                {KeepBell && <span className="pit-row-bell"><KeepBell kind="vrata" id={u.v.id} lang={lang} size={18} /></span>}
          ...
          {groups.map((g) => {
            ...
            return (
              <div className={"pit-group" + (on ? " on" : "")} key={g.id}>
                {KeepBell && <span className="pit-group-bell"><KeepBell kind="tarpana" id={g.id} lang={lang} size={18} /></span>}
  ...
  return (
    <div className="view pit-view scroll">
      <div className="topbar">
        <button className="icon-btn" onClick={() => go(backView || "calendar")} aria-label={STUTI_L.a("aBack")}><Icon name="back" /></button>
...
export { TarpanaWhy, PitruCalendarView, PitruDaysBody, PitruDoor, PiBadge, piT, piPick };

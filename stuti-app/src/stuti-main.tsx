...
function hashView() { return STUTI_ROUTE.view(); }
...
const srStrip = (s) => STUTI_TRANSLIT.strip(s);
const srFold  = (s) => STUTI_TRANSLIT.fold(s);
  ...
  return (
    <div className={"langpick" + (open ? " open" : "")}>
      <button className="langpick-btn" onClick={() => setOpen(o => !o)}
        aria-haspopup="listbox" aria-expanded={open} aria-label={STUTI_L.a("aReadScript")}>
...
function App() {
  ...
  const [uiLang, setUiLangRaw] = useStateM(() => {
    ...
    try { if (!STUTI_PREFS.get().onboarded) rd = "roman"; } catch (e) {}
  ...
  const [panelOpen, setPanelOpen] = useStateM(false);
  const [panelPin, setPanelPin] = useStateM(() => localStorage.getItem("stuti-rdbar-pin") === "1");
  ...
  else if (rv === "daily") body = <NityaView key="daily" go={go} lang={lang} showPractices={false} openRemind={() => setRemindOpen(true)} initLens={route.lens} initLensAt={route.lensAt} />;
  ...
  else if (rv === "pitru") body = <PitruCalendarView key="pitru" go={go} lang={lang} backView={route.from || "calendar"} />;
  else if (rv === "pitruRegister") body = <PitruRegisterView key="pitruRegister" go={go} lang={lang} backView={route.from || "calendar"} />;
  ...
  const appPanelBody = (
    <React.Fragment>
      <button className="icon-btn" onClick={() => { if (!panelPin) setPanelOpen(false); go("settings", { from: route.view }); }}
        aria-label={STUTI_L.t("settings", lang)}>
        <Icon name="gear" size={20} />
      </button>
      <ScriptSeg lang={lang} setLang={setLang} />
      <div className="rd-size brand-size" role="group" aria-label={STUTI_L.a("aTextSize")}>
        <button onClick={() => bumpUi(-1)} disabled={uiScale <= UI_SIZES[0]} aria-label={STUTI_L.a("aSmaller")}>−</button>
        <button onClick={() => bumpUi(1)} disabled={uiScale >= UI_SIZES[UI_SIZES.length - 1]} aria-label={STUTI_L.a("aLarger")}>+</button>
      </div>
    ...
    <div className="stage">
      <div className="app" data-screen-label={route.view}>
        ...
        {appPanel && panelOpen && !panelPin && (
          <div className="rd-toppanel-scrim" onClick={() => setPanelOpen(false)}>
            <div className="rd-toppanel is-drop" role="dialog" aria-label={STUTI_L.t("settings", lang)} onClick={e => e.stopPropagation()}>{appPanelBody}</div>
          </div>
        )}
      ...
      {TweaksPanel && (
        <TweaksPanel title="Tweaks">
          <TweakSection label={STUTI_L.t("library", "roman")} />
          <TweakRadio label="Opens on" value={t.defaultLens} options={["Deity", "Type"]} onChange={(v) => setTweak("defaultLens", v)} />
          <TweakRadio label="Deity tile" value={t.deityTile} options={["Seal", "Full picture"]} onChange={(v) => setTweak("deityTile", v)} />
          <TweakRadio label="Seal" value={t.seal} options={["Current", "Ratna"]} onChange={(v) => setTweak("seal", v)} />
          <TweakSection label="Colour" />
          <TweakRadio label="Palette" value={t.palette} options={["Current", "Warm", "Unified"]} onChange={(v) => setTweak("palette", v)} />
          <TweakRadio label="Glyphs" value={t.glyphs} options={["Ink", "Pigment"]} onChange={(v) => setTweak("glyphs", v)} />
        </TweaksPanel>
      )}
    </div>
  );

...
import { KshanaCard } from "./stuti-kshana";
...
import { TarpanaWhy, piT } from "./stuti-pitru";
...
function usePanchangaVM(lang, sk, deity) {
  ...
  const kshList = STUTI_KSHANA ? STUTI_KSHANA.forDay(pa.date, loc, pa) : [];
  ...
  const skSegs = STUTI_SANKALPA
    ? STUTI_SANKALPA.segs({ date: now, loc, pa, sk, deity, saura })
    : [
    seg("ॐ श्री", "Oṃ Śrī"),
...
function TithiAhead({ go, lang }) {
  ...
  const TT = STUTI_TITHIS, V = STUTI_VRATA, PA = AKSHARA_PANCHANGA;
  const { loc } = useLoc();
          ...
          <div key={r.id} className={"ta-row" + (k.tone === "grave" ? " grave" : "")}>
            ...
            {isOpen && (
              <div className="ta-open">
                <div className="ta-line"><span>{ttAheadT("rule", L)}</span><b>{STUTI_TITHIS.ruleText(r)[T === "tel" ? "tel" : T === "roman" ? "roman" : "deva"]}</b></div>
...
function ParvaOne({ hit, go, lang }) {
  ...
  return (
    <div className={"parva-card" + (hit.eve ? " is-eve" : "") + (v.kind === "tarpana" ? " is-tarpana" : "")}
      ...
      {v.kind === "tarpana" && !hit.eve && TarpanaWhy && (
        <button className="icon-btn parva-why" aria-label={piT ? piT("why", L) : "Why this date?"}
          title={piT ? piT("why", L) : "Why this date?"}
      ...
      {why && TarpanaWhy && <TarpanaWhy id={v.id} date={hit.date} lang={L} onClose={() => setWhy(false)} />}
  ...
  return (
    <div className="view home home-a scroll" style={deityStyle(deity)}>
      ...
      <section className="rhb-card">
        ...
        {ksh && <KshanaCard k={ksh} date={vm.pa.date} lang={L} onClose={() => setKsh(null)} />}

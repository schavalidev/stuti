...
export const STUTI_BUILD = (function () {
  ...
  function diagnostics() {
    ...
    return [
      ...
      "place: "  + g(0, () => {
        const id = STUTI_LOC.getLocId();
        if (id === "detected") { const d = STUTI_LOC.getDetected(); return d ? d.city + " (detected)" : "detected"; }
        return id;
      }),
      "tz: " + g(0, () => Intl.DateTimeFormat().resolvedOptions().timeZone + " (" + (-new Date().getTimezoneOffset() / 60) + ")"),
      "saved: " + g(0, () => STUTI_FAVS.list().length) + " · watching: " + g(0, () => STUTI_WATCH.count()),
      "last read: " + g(0, () => { const p = STUTI_PROGRESS.get(); return p ? p.hymnId + " @ line " + p.line + (p.verse != null ? " / verse " + p.verse : "") : "none"; }),
      ...
      "notify: " + g(0, () => (STUTI_NUDGE ? STUTI_NUDGE.permission() : "n/a")),
  ...
  function tally() {
    ...
    return {
      days:  g(() => STUTI_THREAD.streak().days, 0),
      japa:  g(() => Object.keys(JSON.parse(localStorage.getItem("stuti-japa") || "{}"))
                      .reduce((n, id) => n + (STUTI_JAPA.state(id).total || 0), 0), 0),
      plans: g(() => STUTI_PLANS.active().length, 0),
      vows:  g(() => STUTI_VOWS.list().length, 0),
      saved: g(() => STUTI_FAVS.list().length, 0),
    };
  }

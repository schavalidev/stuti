...
function findKeyDeva(s) {
  let t = STUTI_TRANSLIT.toDeva(s || "").replace(/ॐ/g, "ओं").replace(/[\s।॥|0-9०-९౦-౯\-—–,.;:!?'"()]/g, "");
  let out = "";
  for (const ch of t) out += FIND_DEVA_LONG[ch] || ch;
  return out;
}
function findKeyRoman(s) { return STUTI_TRANSLIT.fold(s || "").replace(/[^a-z]/g, ""); }
function findKey(q) {
...
function FindStrip({ lines, hymn, lang, active, onPick, onClose }) {
  ...
  const lineText = (l) => {
    ...
    if (lang === "telugu") { const tdv = v.tdeva ? v.tdeva.split("\n") : null; return (tdv && tdv[l.li]) || STUTI_TRANSLIT.convert(l.deva, "telugu"); }
    return l.deva;
  };

...
const vowTermName = (t, lang, v) => t.custom && v ? STUTI_L.t("vowNDays", lang).replace("{n}", v.days | 0) : lang === "telugu" ? t.tel : lang === "deva" ? t.deva : t.name;
/* tithi index 0–29 → "Śukla Caturthī", "Pūrṇimā", "Kṛṣṇa Aṣṭamī", "Amāvāsyā" */
function vowTithiName(i, lang) {
  const PA = AKSHARA_PANCHANGA, L = STUTI_L;
  ...
  return (
    <div className="ssel">
      <div className="ssel-field">
        ...
        <input aria-label={ariaLabel} placeholder={placeholder || STUTI_L.t("searchHint", STUTI_L.ui())}
      ...
      {showList && (
        <div className="ssel-list">
          ...
          )) : <div className="ssel-empty">{STUTI_L.t("noResults", STUTI_L.ui())}</div>}
        </div>
      )}
    </div>
  );
...
function VowSheet({ lang = "deva", onClose }) {
  ...
  const draft = { hymn, occasion, weekday, tithi, from, to, term, days, start: STUTI_THREAD.dkey() };
        ...
        <div className="vow-form">
          ...
          {o.range && (
            <div className="vow-nest vow-range">
              <label className="vow-date"><span>{L.t("vowFrom", lang)}</span><HouseDate value={from} lang={lang} min={todayKey} onChange={(v) => { setFrom(v); if (v > to) setTo(v); }} /></label>
              <label className="vow-date"><span>{L.t("vowTo", lang)}</span><HouseDate value={to} lang={lang} min={from} onChange={(v) => setTo(v)} /></label>
            </div>
          )}
          </section>

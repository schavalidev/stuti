...
function ShareSheet({ hymn, verse, num, lang, theme, onClose }) {
  ...
  const text = lang === "telugu" ? (verse.tdeva || STUTI_TRANSLIT.convert(verse.deva, "telugu")) : lang === "deva" ? verse.deva : verse.iast;
...
function printStotra(hymn, lang, withMeaning) {
  const L = STUTI_L, TR = STUTI_TRANSLIT;
  ...
  (hymn.verses || []).forEach(v => {
    ...
    const mean = withMeaning ? STUTI_MEAN(v, lang) : "";
...
export { ShareSheet, printStotra, drawVerseCard };

  ...
  return (
    <nav className="tabbar tabbar-4">
      ...
      <div className="rail-head" aria-hidden="true">
        <Flame size={30} />
        <span className="rail-name" style={{ fontFamily: STUTI_L.font(lang) }}>{BRAND_SCRIPT[lang] || "Stuti"}</span>
      </div>
...
function App() {
  ...
  else if (route.view === "daily") body = <NityaView key="daily" go={go} lang={lang} showPractices={false} openRemind={() => setRemindOpen(true)} initLens={route.lens} initLensAt={route.lensAt} />;
        ...
        {route.view === "home" && (
          <div className="brandbar">
            <div className="brand">
              ...
              {BRAND_SCRIPT[lang] ? (
                <React.Fragment>
                  <span className="brand-name" style={{ fontFamily: STUTI_L.font(lang), letterSpacing: 0 }}>{BRAND_SCRIPT[lang]}</span>
            ...
            <span className="brand-shri" style={{ fontFamily: STUTI_L.font(lang) }}>{SHRI[lang] || SHRI.deva}</span>
            <div className="brandbar-actions">
              <button className="icon-btn" onClick={toggleTheme} aria-label="Day or night">
                <Icon name={theme === "night" ? "sun" : "moon"} />
              </button>
              <button className="icon-btn" onClick={() => go("settings")} aria-label={STUTI_L.t("settings", lang)}>
                <Icon name="gear" />
              </button>
            </div>
          </div>

...
function useTithis() { const [, f] = useStateTT(0); useEffectTT(() => STUTI_TITHIS.subscribe(() => f((x) => x + 1)), []); return STUTI_TITHIS; }
  ...
  return (
    <div className="tt-placewrap">
      <button type="button" className="tt-place" onClick={() => { setQ(""); setOpen(true); }}>
        <span>{label}</span><Icon name="search" size={15} />
      </button>
      {open && (
        <React.Fragment>
...
function ClockField({ value, onChange, lang }) {
  ...
  return (
    <div className="tt-clock">
      ...
      {cur != null && <button type="button" className="tt-clock-clear" onClick={() => onChange("")}><Icon name="close" size={14} /></button>}
    </div>
  );
}
...
function TithiSheet({ lang = "deva", rec, seedDate, onClose }) {
  const L = STUTI_L, T = STUTI_TITHIS, P = AKSHARA_PANCHANGA;
  const { loc } = useLoc();
  ...
  const ttSpell = (r, k) => {
    const TR = STUTI_TRANSLIT; const s = (r || "").trim();
    if (!TR || !s) return "";
  ...
  const LOCS = (AKSHARA_PANCHANGA && AKSHARA_PANCHANGA.locations) || [];
  ...
  const next = useMemoTT(() => {
    try { const V = STUTI_VRATA; return V.nextDate(T.asVrata({ id: "x", name, kind, masa, ti })); } catch (e) { return null; }
  }, [masa, ti, kind]);
  ...
  const save = () => {
    ...
    try { kpArmReminders && kpArmReminders(); } catch (e) {}
    onClose();
  };
          ...
          <div className="pd-body scroll">
            ...
            {["deva", "tel"].map((k) => (
              <label className="acc-field" key={k} style={{ marginTop: 8 }}>
                ...
                <input value={nm[k]} onChange={(e) => setNmK(k, e.target.value)} style={{ fontFamily: NM_FONT[k] }}
                  onBlur={() => { const TR = STUTI_TRANSLIT, v = nm[k]; if (!TR || !v || !/[\u0900-\u097F\u0C00-\u0C7F]/.test(v)) return; const d = TR.toDeva(v); const w = k === "tel" ? TR.convert(d, "telugu") : d; if (w !== v) setNmK(k, w); }} />
              </label>
            ))}
            ...
            <div className="tt-row">
              <label className="acc-field">
                ...
                <DayPick value={from} lang={lang} onChange={onDate} label={ttT("fromDate", lang)} place="down" />
              </label>
              <label className="acc-field">
                <span>{ttT("place", lang)}</span>
                <PlaceField locs={LOCS} value={placeId} onChange={onPlace} onPick={onPickPlace}
            ...
            {kind !== "other" && (
              <>
                <label className="acc-field" style={{ marginTop: 12 }}>
                  ...
                  <ClockField value={time} onChange={onTime} lang={lang} />
                </label>
...
function MyTithisCard({ lang = "deva", onJump }) {
  const T = useTithis(), V = STUTI_VRATA;
  ...
  return (
    <section className="tt-card">
      <div className="tt-head">
        <div className="eyebrow" style={{ color: "var(--accent-ink)" }}>{ttT("cap", lang)}</div>
        <button className="tt-add" onClick={() => setSheet({})}><Icon name="plus" size={15} />{ttT("add", lang)}</button>
      </div>
        ...
        <div className="tt-list">
          {rows.map(({ r, v, d, away }) => (
            <button key={r.id} className="tt-row" onClick={() => setSheet({ rec: r })}>
              ...
              {d && <span className="tt-row-date" onClick={(e) => { if (onJump) { e.stopPropagation(); onJump(d); } }}>{d.toLocaleDateString(lang === "telugu" ? "te-IN" : lang === "deva" ? "hi-IN" : "en-IN", { day: "numeric", month: "short" })}<small>{away === 0 ? STUTI_L.t("vrataToday", lang) : away === 1 ? STUTI_L.t("vrataTomorrow", lang) : STUTI_L.t("vrataInDays", lang).replace("{n}", away)}</small></span>}
            </button>
          ))}
        </div>
      )}
...
export { PlaceField, ClockField, TithiSheet, MyTithisCard, useTithis, ttT };

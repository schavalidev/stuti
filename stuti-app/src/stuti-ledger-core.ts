...
/* ============================================================
   ...
   ============================================================ */
export const STUTI_LEDGER = (function () {
  const MALA = 108, DAY = 86400000;
  const T = () => STUTI_THREAD;
  ...
  function dueRecite(d) {
    let ids = [];
    try { ids = STUTI_FAVS.list().concat(STUTI_FAVS_WEEK.list(d.getDay())); } catch (e) {}
  ...
  const open = () => {
    let plans = 0, nomus = 0;
    try { plans = STUTI_PLANS.active().length; } catch (e) {}
    try { nomus = STUTI_KEEP.list().filter((k) => k.kind === "nomu" && !k.kept).length; } catch (e) {}
    return { plans, nomus };
  };

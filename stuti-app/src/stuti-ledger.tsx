...
const lgName = (o, lang) => !o ? "" : (pick3 ? pick3(o.name, lang) : (o.name && (o.name.roman || o.name.tel)) || "");
...
function LedgerDetail({ lang, det, span, onOpen }) {
  const L = STUTI_L, S = STUTI, font = L.font(lang);
  ...
  const keepName = (k) => { let s = null; try { s = STUTI_KEEP.subject(k); } catch (e) {} return lgName(s, lang) || k.ref; };
...
function LedgerPeek({ lang, title, eyebrow, days, onOpen, onClose }) {
  const L = STUTI_L, LG_ = LG(), font = L.font(lang);
          ...
          <div className="pd-body scroll lg-sheet-body">
            {figs.length > 0 && (
              <div className="lg-peek-figs">
                {figs.map((f) => <span key={f.k} className="lg-peek-fig"><Icon name={f.icon} size={15} /><b style={{ fontFamily: font }}>{f.n}</b><span>{f.label}</span></span>)}
              </div>
            )}
...
export { MonthHeat, DayCells, YearBars, LedgerDetail, DayPeekSheet, LedgerPeek, ReciteTracker, JapaTracker, LearnTracker, LedgerLens, Thread, useLedger };

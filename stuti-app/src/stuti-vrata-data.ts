import { STUTI_BACKLOG_EXTRA } from "./stuti-backlog-data";
import { STUTI_CYCLE_EXTRA } from "./stuti-cycle-data";
import { STUTI_DIKSHA_EXTRA } from "./stuti-diksha-data";
...
export const STUTI_VRATA = (function () {
  ...
  [STUTI_CYCLE_EXTRA, STUTI_DIKSHA_EXTRA, STUTI_BACKLOG_EXTRA].forEach((mod) => {
    if (!mod) return;
  ...
  /* every record takes its type, evidence grade and publication status from
     ...
     keeps it */
  if (STUTI_PROV) vratas.forEach((v) => STUTI_PROV.stamp(v, false));
  ...
  function upFingerprint() {
    ...
    try { const p = STUTI_PREFS.get(); parts.push(p.masaSystem, p.reckoning, p.ayanamsa, p.mana, p.tradition || ""); } catch (e) {}
    try {
      const list = (STUTI_TITHIS && STUTI_TITHIS.list()) || [];

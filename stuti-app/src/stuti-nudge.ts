...
/* ============================================================
   ...
   ============================================================ */
export const STUTI_NUDGE = (function () {
  ...
  function body(lg) {
    ...
    try {
      ...
      if (!h) { for (const id of STUTI_FAVS.list()) { const x = S.hymnById(id); if (x && x.verses && x.verses.length) { h = x; break; } } }
      if (!h) { const t = todayInfo && todayInfo(); h = t ? t.hymn : S.hymns[0]; }
    } catch (e) {}
  ...
  function cues(from) {
    try { return STUTI_CUES.ring(STUTI_CUES.ctx(from || new Date())); }
    catch (e) { return []; }
  }
  ...
  function digestParts(items, lg) {
    const L = STUTI_L, S = STUTI, TR = STUTI_TRANSLIT;
    const parts = [];
    (items || []).forEach((it) => {
      if (it.kind === "vow" || it.kind === "plan") {
        ...
        try {
          const k = STUTI_KEEP.byId(it.ref), s = k && STUTI_KEEP.subject(k); if (!s) return;
  ...
  function post(cue) {
    ...
    const lg = lang(), L = STUTI_L, P = AKSHARA_PANCHANGA, S = STUTI_SANDHYA;
    ...
    if (cue.kind === "sandhya") {
      ...
      if (parts.length) {
        text = parts.join(" · ");
        if (one.length === 1) hymn = STUTI.hymnById(one[0].hymn) || null;
      } else {
  ...
  function start() {
    try { STUTI_PREFS.subscribe(arm); } catch (e) {}
    try { STUTI_LOC.subscribe(arm); } catch (e) {}

...
import { STUTI_BIND, STUTI_L, STUTI_MEAN, STUTI_SEC } from "./stuti-i18n";
import { Flame, Icon } from "./stuti-icons";
import { RecMasked, WordRun } from "./stuti-reader";
...
function FlowVerse({ hymn, v, vi, numbered, lang, showMeaning, label, sub, sec, on, activeLi, word, lit, masked, hint, ritualOn, onRitual, onPick, onWord, onOpenNames, setMark }) {
  ...
  return (
    <div className={"flow-v" + (on ? " fl-v-on" + (numbered ? " at" : "") : "")} data-vi={vi}
      ...
      {label && (
        <React.Fragment>
          <div className={"verse-sec" + (lang !== "roman" ? " tel" : "")} style={{ fontFamily: STUTI_SEC.font(sec, lang) }}>{label}</div>
          {sub && <div className="verse-sec-sub">{sub}</div>}
          {sec && sec.ritual && ritualOn && (
            <button className="ritual-off" onClick={e => { e.stopPropagation(); onRitual && onRitual(); }}>{STUTI_L.t("ritualHide", lang)}</button>
          )}
        </React.Fragment>
      )}
      {v.sp && (
      ...
      <div className="flow-body">
        ...
        {showMeaning && !masked && STUTI_MEAN(v, lang) && (
          <div className="flow-mean" style={{ fontFamily: STUTI_MEAN.font(v, lang) }}>
            {STUTI_MEAN(v, lang)}
          </div>
        )}
    ...
    <div className={"flow" + (lit && at ? " flow-lit" : "")}>
      {hymn.verses.map((v, vi) => {
        ...
        if (ritual && ritual.has(vi)) {
          ...
          for (let j = vi; j < hymn.verses.length && ritual.has(j); j++) {
            const nm = STUTI_SEC(hymn.sections[hymn.verses[j].s], lang);
            if (nm && names.indexOf(nm) < 0) names.push(nm);
          }
          return (
            <div key={vi} className="ritual-band" ref={el => (marks.current[vi] = el)}>
              <div className="ritual-band-name" style={{ fontFamily: STUTI_SEC.font(sec, lang) }}>{names.join(" · ")}</div>
              <div className="ritual-band-note">{STUTI_L.t("ritualNote", lang)}</div>
              <button className="ritual-band-btn" onClick={e => { e.stopPropagation(); onRitual && onRitual(); }}>{STUTI_L.t("ritualShow", lang)}</button>
            </div>
          );
        }
        ...
        return (
          <FlowVerseMemo key={vi} hymn={hymn} v={v} vi={vi} numbered={hymn.verses.length > 1}
            ...
            label={newSec ? STUTI_SEC(sec, lang) : null}
            sub={newSec ? STUTI_SEC.sub(sec, lang) : null} sec={sec}
      ...
      <div className="reader-end">
        ...
        <div style={{ fontFamily: lang === "telugu" ? "var(--font-telugu)" : lang === "deva" ? "var(--font-deva)" : "var(--font-display)", fontSize: 20, color: "var(--accent-ink)" }}>
          {STUTI_L.t("shubham", lang)}
        </div>
      </div>
      <div className="flow-tail">{STUTI_L.t("flowTapHint", lang)}</div>
    </div>
  );
}
...
export { FlowTextMemo as FlowText };

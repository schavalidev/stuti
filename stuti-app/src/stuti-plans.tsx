...
import { STUTI_L, STUTI_MEAN } from "./stuti-i18n";
...
function planVerseText(v, lang) {
  if (lang === "telugu") return v.tdeva || (STUTI_TRANSLIT ? STUTI_TRANSLIT.convert(v.deva, "telugu") : v.deva);
...
function PlanEntryCard({ go, lang = "deva" }) {
  const S = STUTI, L = STUTI_L, P = STUTI_PLANS;
    ...
    <div className="view libhub scroll">
      ...
      <div className="lens-pad">
        ...
        <div className="ssel">
          <div className="ssel-field">
            ...
            <VoiceButton lang={lang} size={16} onInterim={setQ} onResult={setQ} />
            {q && <button type="button" className="ssel-x" aria-label={STUTI_L.a("aClose")} onClick={() => setQ("")}><Icon name="close" size={15} /></button>}
          </div>
        </div>
      ...
      {sel.size > 0 && (
        <div className="plan-selbar">
          ...
          <button className="plan-selbar-x" aria-label={STUTI_L.a("aClose")} onClick={() => setSel(new Set())}><Icon name="close" size={16} /></button>
        </div>
      )}
    </div>
  );
...
function PlanView({ hymnId, go, lang = "deva", backView = "daily" }) {
  ...
  const doneToday = plan.last === STUTI_THREAD.dkey() && plan.done.indexOf(day) >= 0;
...
function PlanDay({ hymn, deity, day, meta, go, lang, backView = "daily" }) {
  ...
  if (finished) {
    return (
      <div className="view japa scroll" style={{ "--deity-hue": deity.hue }}>
        <div className="topbar">
          <button className="icon-btn" onClick={back} aria-label={STUTI_L.a("aBack")}><Icon name="back" /></button>
  ...
  const meaning = STUTI_MEAN(v, lang);
...
export { PlanEntryCard, PlansView, PlanView, MaskedVerse, LearnButton };

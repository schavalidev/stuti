...
import { MasaDetail, MasaLens } from "./stuti-masa";
import { NomuDetail, NomuLens } from "./stuti-nomu";
import { ParayanaDetail, ParayanaLens } from "./stuti-parayana";
import { LearnButton } from "./stuti-plans";
import { STUTI_PREFS } from "./stuti-prefs";
import { FestivalLens, VrataDetail, VrataLens } from "./stuti-vrata";
...
function HymnRow({ h, go, lang, showSeal = true, i = 0, from = "browse" }) {
  ...
  return (
    <div className={"hymn-card" + (h.catalog ? " hymn-card-soon" : "")}
      ...
      {!h.catalog && <LearnButton id={h.id} size={22} go={go} from="browse" />}
...
function BuildStamp({ lang, noReset }) {
  ...
  return (
    <div className="build-foot">
      <button className="build-chip" onClick={tap} aria-label={STUTI_L.a("aCopyBuild")}>
...
function LibraryHub({ go, lang = "deva", tileMode = "seal", lens = "deity", setLens, sub = null, setSub }) {
  ...
  if (sub) {
    ...
    if (sub.kind === "parayana") return <ParayanaDetail parayanaId={sub.key} go={go} lang={lang} onBack={back} />;
    if (sub.kind === "vrata") return <VrataDetail vrataId={sub.key} go={go} lang={lang} onBack={back} />;
    if (sub.kind === "nomu") return <NomuDetail nomuId={sub.key} go={go} lang={lang} onBack={back} />;
    if (sub.kind === "masa") return <MasaDetail masaId={sub.key} go={go} lang={lang} onBack={back} onOpen={(kind, key) => setSub({ kind, key })} />;
  }
  ...
  else if (lens === "vrata") body = <VrataLens go={go} lang={lang} onOpen={(k) => setSub({ kind: "vrata", key: k })} />;
  else if (lens === "festival") body = <FestivalLens go={go} lang={lang} onOpen={(k) => setSub({ kind: "vrata", key: k })} />;
  else if (lens === "nomu") body = <NomuLens go={go} lang={lang} onOpen={(k) => setSub({ kind: "nomu", key: k })} />;
  else if (lens === "parayana") body = <ParayanaLens go={go} lang={lang} onOpen={(k) => setSub({ kind: "parayana", key: k })} />;
  else if (lens === "masa") body = <MasaLens go={go} lang={lang} onOpen={(k) => setSub({ kind: "masa", key: k })} />;
...
export { useLensSwipe, useLensArrange, LibraryHub, HymnRow, pick3, BuildStamp, ResetPanel };

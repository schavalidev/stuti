...
const kPick = (o, lang) => STUTI_KSHANA.pick(o, lang);
...
const kTime = (min, lang) => {
  const P = AKSHARA_PANCHANGA, T = STUTI_KSHANA.T;
...
function KshanaCard({ k, date, lang = "deva", onClose }) {
  const L = STUTI_L, KS = STUTI_KSHANA;
  if (!k) return null;
            ...
            {k.sources && k.sources.length > 0 && (
              <div className="pit-src">
                ...
                {k.sources.map((s, i) => s.url
                  ? <a key={i} href={s.url} target="_blank" rel="noreferrer noopener" className="pit-src-row"><span>{s.label}</span><Icon name="chev" size={14} /></a>
...
export { KshanaCard, KshanaLine, kPick, kTime };

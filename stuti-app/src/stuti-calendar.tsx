...
function CalendarView({ go, lang = "deva" }) {
  ...
  const rows = [
    ...
    { k: "limbMasa", v: pick(masaShown(pa)).main
  ...
  return (
    <div className="view cal-view scroll">
      {(() => {
        ...
        const rv = (RTU_VIS || {})[monthPa.ritu.iast];
        /* a transition month carries a mild mix of both seasons, as the Māsa lens does */
        const mName = masaShown ? masaShown(monthPa) : monthPa.masa;
      ...
      <div className="topbar">
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 4, marginLeft: "auto", marginRight: 26 }}>
          <button className={"icon-btn" + (calSearchOpen ? " on" : "")} onClick={() => { setCalSearchOpen((o) => !o); setCalQ(""); }} aria-label={STUTI_L.a("aSearch")} aria-expanded={calSearchOpen}><Icon name={calSearchOpen ? "close" : "search"} size={19} /></button>
          <LocationControl />
        </div>
      </div>
      ...
      {calSearchOpen && (
        <div className="cal-search">
          <div className="search-field">
            ...
            <VoiceButton lang={lang} onInterim={setCalQ} onResult={setCalQ} />
            {calQ && <button className="search-clear" onClick={() => setCalQ("")} aria-label={STUTI_L.a("aClearSearch")}>×</button>}
          </div>
        </div>
      )}
      ...
      <div className="cal-nav">
        <button className="icon-btn" onClick={() => step(-1)} aria-label={STUTI_L.a("aPrevMonth")}><Icon name="prev" size={20} /></button>
        ...
        <button className="icon-btn" onClick={() => step(1)} aria-label={STUTI_L.a("aNextMonth")}><Icon name="next" size={20} /></button>
      </div>

      {pickerOpen && (
        <div className="cal-picker">
          <button className="icon-btn cal-picker-close" onClick={() => setPickerOpen(false)} aria-label={STUTI_L.a("aClose")}><Icon name="close" size={16} /></button>
          <div className="cal-picker-year">
            <button className="icon-btn" onClick={() => setPickYear(y => y - 1)} aria-label={STUTI_L.a("aPrevYear")}><Icon name="prev" size={18} /></button>
            ...
            <button className="icon-btn" onClick={() => setPickYear(y => y + 1)} aria-label={STUTI_L.a("aNextYear")}><Icon name="next" size={18} /></button>
          </div>

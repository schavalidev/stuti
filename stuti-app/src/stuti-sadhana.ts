...
/* ============================================================
   ...
   ============================================================ */
export const STUTI_THREAD = (function () {
...
export const STUTI_JAPA = (function () {
  ...
  const addOn = (id, n, key) => {
    ...
    if (n > 0) STUTI_THREAD.mark("j", null, n, k);
    save();
    return s.days[k];
  };
  return {
    ...
    history: (id, n) => {
      ...
      for (let i = n - 1; i >= 0; i--) { const k = STUTI_THREAD.dkey(new Date(t - i * day)); out.push({ key: k, n: s.days[k] || 0, today: i === 0 }); }
      return out;
    },
...
/* ============================================================
   ...
   ============================================================ */
export const STUTI_PLANS = (function () {
  ...
  return {
    ...
    start: (id) => { if (!db[id]) { db[id] = { started: STUTI_THREAD.dkey(), done: [], last: "" }; save(); } },
    ...
    completeDay: (id, day, hymn) => {
      ...
      p.done.push(day); p.last = STUTI_THREAD.dkey();
      if (p.done.length >= meta(hymn).days) p.finished = true;
      STUTI_THREAD.mark("p", "plan:" + id + ":" + day);
      save();
    },
    subscribe: (fn) => { subs.add(fn); return () => subs.delete(fn); },
  };
...
/* ============================================================
   ...
   ============================================================ */
export const STUTI_VOWS = (function () {
  ...
  return {
    ...
    keep: (id) => { const v = list.find(x => x.id === id); if (!v) return; const k = STUTI_THREAD.dkey(); if (v.kept.indexOf(k) < 0) { v.kept.push(k); STUTI_THREAD.mark("p", "vow:" + id + ":" + k); save(); } },
    keptToday: (v) => v.kept.indexOf(STUTI_THREAD.dkey()) >= 0,

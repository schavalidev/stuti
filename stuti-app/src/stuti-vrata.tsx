...
import { KeepBell, KeepSpan } from "./stuti-keep";
...
const vIsVratam = (v) => (STUTI_PROV ? STUTI_PROV.isType(v, "vratam", "cycle", "diksha") : v.kind === "vratam");
...
const vIsVow = (v) => !!(v.floating || v.optional || v.everyMonth || (STUTI_PROV && STUTI_PROV.isType(v, "diksha")));
const vVowGroup = (v) => (v.optional && !v.floating ? "vrataGrpAny" : v.everyMonth ? "vrataGrpFortnight" : STUTI_PROV && STUTI_PROV.isType(v, "diksha") ? "vrataGrpDiksha" : "vrataGrpCycle");
...
function VowRow({ v, lang, onOpen, no, entry }) {
  ...
  return (
    <div className="gs-en">
      <KeepBell kind="vrata" id={v.id} lang={lang} />
      <button className="gs-main" onClick={() => onOpen(v.id)}>
        <span className="gs-no" style={{ fontFamily: font }}>{String(no)}</span>
    ...
    <div className="view libhub scroll" style={deityStyle(d, { flex: 1 })}>
      ...
      <div className="lens-pad">
        <header className="vr-hero">
          ...
          {KeepSpan && STUTI_KEEP.spanOf(v.id) && <KeepSpan vrataId={v.id} lang={lang} />}
        ...
        {v.prov && STUTI_PROV && <div className="vr-source vr-prov">{STUTI_PROV.typeName(v, lang)} · {STUTI_PROV.line(v, lang)}</div>}

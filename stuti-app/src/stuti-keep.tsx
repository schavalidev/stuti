...
function kpSpanLine(K, k, lang) {
  const L = STUTI_L, s = K.spanState(k); if (!s) return "";
  ...
  return (
    <div className="keep-span">
      <div className="keep-span-row">
        ...
        <HouseDate value={k.start} onChange={(key) => { if (key) K.setStart(k.id, key); }} lang={lang} ariaLabel={L.t("keepBegunOn", lang)} />
      </div>
              ...
              <div key={k.id} className={"vow keep-row" + (due && !due.done ? " due" : "")} style={{ "--deity-hue": d ? d.hue : 36 }}>
                <div className="vow-top">
                  <div className="vow-body">
                    <div className="vow-occ">{k.kind === "tarpana" ? (piT ? piT("cap", lang) : "Pitṛ") : k.kind === "vrata" ? L.t("lensVrata", lang) : L.t("lensNomu", lang)}{status ? " · " + status : ""}</div>
...
export { KeepSpan, KeepBell, KeepCard, KeepMonthSheet, NomuTracker, stutiToast };

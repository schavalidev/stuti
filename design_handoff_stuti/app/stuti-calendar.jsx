...
function CalendarView({ go, lang = "deva" }) {
  const P = window.AKSHARA_PANCHANGA, L = window.STUTI_L, TR = window.STUTI_TRANSLIT, V = window.STUTI_VRATA, TP = window.STUTI_TARPANA;
  ...
  React.useEffect(() => window.STUTI_TITHIS ? window.STUTI_TITHIS.subscribe(() => bumpMy((n) => n + 1)) : undefined, []);
  ...
  return (
    <div className="view cal-view scroll">
      ...
      <div className="cal-lenses" role="tablist">
        {[["panchanga", L.t("calendar", lang)], ["tithis", window.ttT("cap", lang)], ["pitru", window.piT ? window.piT("cap", lang) : "Pitṛ"], ["tarpana", window.piT ? window.piT("title", lang) : "Tarpaṇa"]].map(([id, label]) => (
            ...
            <div className="cal-detail-head">
              <div className="cal-detail-headbody">
                ...
                <div className={"cal-detail-tithi" + (selMine.length ? "" : " is-none")} style={{ fontFamily: calFont(lang) }}>
                  {selMine.length ? selMine.map((f) => nameOf(f.name)).join(" · ") : window.ttT("noneToday", lang)}
                </div>
              </div>
              <div className="cal-detail-acts">
                {!sameDay(sel, today) && (
                ...
                <button className="cal-today-btn tt-addhere-sm cal-add-here" onClick={() => setTithiSheet({ seedDate: sel })}>
                  <Icon name="plus" size={15} />{window.ttT("addHere", lang)}
                </button>
              </div>
            </div>
          </section>
          <window.MyTithisCard lang={lang} onJump={jumpTo} />
        </React.Fragment>
      )}
      ...
      {lens === "pitru" && (
        <React.Fragment>
          ...
          <div className="cal-pit-doors">
            {window.PitruRegisterDoor && <window.PitruRegisterDoor lang={lang} onOpen={() => go("pitruRegister", { from: "calendar" })} />}
          </div>
        </React.Fragment>
      )}
      ...
      {lens === "tarpana" && window.PitruDaysBody && (
        <React.Fragment>
          <p className="pit-lede pit-lede-sm cal-tarp-lede">{window.piT("lede", lang)}</p>
          <window.PitruDaysBody lang={lang} />
        </React.Fragment>
      )}
      {tithiSheet && <window.TithiSheet lang={lang} rec={tithiSheet.rec} seedDate={tithiSheet.seedDate} onClose={() => setTithiSheet(null)} />}
      </React.Fragment>
      )}

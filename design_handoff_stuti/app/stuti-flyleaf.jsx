...
function GotraField({ value, onChange, lang, id }) {
  const L = window.STUTI_L, SK = window.SK_CONST, TR = window.STUTI_TRANSLIT;
...
function FlyleafForm({ sk, lang, unknown }) {
  const L = window.STUTI_L, t = (k) => L.t(k, lang);
...
function KnownClauses({ lang, sk }) {
  const L = window.STUTI_L, PA = window.AKSHARA_PANCHANGA, TR = window.STUTI_TRANSLIT, DS = window.STUTI_DESA;
  const { loc } = window.useLoc();
  let pa = null;
  try { pa = PA.forDay(new Date(), loc, { instant: true }); } catch (e) {}
  if (!pa) return null;
  const samv = window.samvatsaraFor(new Date());
  const sc = (deva, iast) => lang === "telugu" ? TR.convert(deva, "telugu") : lang === "roman" ? iast : deva;
  const rows = [
    ["limbSamvatsara", sc(samv[1], samv[0])],
    ["limbMasa", sc(window.masaShown(pa).deva, window.masaShown(pa).iast)],
...
function SankalpaSheet({ vm, sk, L, go }) {
  const I = window.STUTI_L, t = (k) => I.t(k, L);
  const F = window.STUTI_FLYLEAF, DS = window.STUTI_DESA;
  const { loc } = window.useLoc();
  const SK = window.SK_CONST;
      ...
      {sk.ready && (
        <div className="fl-tail">
          <div className="sk-field">
            ...
            <window.Picker value={sk.karma} onChange={sk.setKarma} ariaLabel={t("intentionL")}
...
function FlyleafSettings({ lang }) {
  ...
  const sk = useFlyleaf();
  return (
    <window.SetSection title={L.t("flTitle", lang)} note={L.t("flDeviceNote", lang)}>
      <div className="fl-set"><FlyleafForm sk={sk} lang={lang} /></div>
    </window.SetSection>
  );
}

Object.assign(window, { useFlyleaf, GotraField, FlyleafForm, KnownClauses, SankalpaSheet, FlyleafSettings, flFont });

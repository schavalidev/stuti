...
function useWatch(id) {
  const [on, setOn] = useStateW(() => window.STUTI_WATCH.has(id));
  useEffectW(() => window.STUTI_WATCH.subscribe(() => setOn(window.STUTI_WATCH.has(id))), [id]);
  return [on, () => window.STUTI_WATCH.toggle(id)];
}
  ...
  return (
    <div className="view reader" style={deityStyle(deity, { flex: 1 })}>
      <div className="topbar reader-topbar">
        <button className="icon-btn" onClick={() => go(backView, { deity: deity.id, from: retView })} aria-label={window.STUTI_L.a("aBack")}><Icon name="back" /></button>
...
function WatchCard({ go, lang = "deva" }) {
  const S = window.STUTI, L = window.STUTI_L;
  const [ids, setIds] = useStateW(() => window.STUTI_WATCH.list());
  const [open, setOpen] = useStateW(false);
  useEffectW(() => window.STUTI_WATCH.subscribe(setIds), []);
          ...
          {items.map((h) => {
            ...
            return (
              <div className="watch-row" key={h.id} style={deityStyle(d)}>
                ...
                <button className="watch-row-x" onClick={() => window.STUTI_WATCH.remove(h.id)} aria-label={L.t("removeWatch", lang)}>×</button>
              </div>
            );
          })}
        </div>
...
Object.assign(window, { AwaitingText, WatchCard, useWatch });

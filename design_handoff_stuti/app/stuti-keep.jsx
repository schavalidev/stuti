...
const kpMasaName = (m, lang) => m === "any" ? window.STUTI_L.t("keepAnyMonth", lang) : kpPick(m.name, lang);
...
function useKeep() { const [, f] = useStateK(0); useEffectK(() => window.STUTI_KEEP.subscribe(() => f((x) => x + 1)), []); return window.STUTI_KEEP; }
...
function kpArmReminders() {
  try { const r = window.STUTI_PREFS.get().remind || {}; if (!r.on) window.STUTI_PREFS.setRemind({ on: true, time: r.time || "06:00" }); } catch (e) {}
  try { const N = window.STUTI_NUDGE; if (N && N.permission() === "default") N.ask(); } catch (e) {}
}
...
function KeepBell({ kind, id, lang, size = 17 }) {
  ...
  return (
    <React.Fragment>
      ...
      {ask && <window.OverlayPortal><KeepMonthSheet lang={lang} onClose={() => setAsk(false)}
        onPick={(idx) => { K.add(kind, id, { masa: idx }); kpArmReminders(); setAsk(false); stutiToast(L.t("keepAdded", lang)); }} /></window.OverlayPortal>}
    </React.Fragment>
  );
}
...
function KeepCard({ go, lang = "deva" }) {
  const K = useKeep(), L = window.STUTI_L, MA = window.STUTI_MASA;
  ...
  return (
    <div className="vows keep">
      ...
      {pickFor && <window.OverlayPortal><KeepMonthSheet lang={lang} current={(K.byId(pickFor) || {}).masa} onClose={() => setPickFor(null)}
        onPick={(idx) => { K.setMasa(pickFor, idx); setPickFor(null); }} /></window.OverlayPortal>}
    </div>
  );
}
...
function NomuTrackerRow({ k, lang, go, K, eng }) {
  const L = window.STUTI_L, MA = window.STUTI_MASA, S = window.STUTI;
  const [armed, setArmed] = useStateK(null);
    ...
    <article className={"vow keep-row nt-row" + (due && !due.done ? " due" : "")} style={{ "--deity-hue": d ? d.hue : 36 }}>
      ...
      {k.mode === "daily" && !k.kept && (
        <div className="nt-track">
          <window.Thread read={(dd) => ({ v: k.ticks.indexOf(K.dkey(dd)) >= 0 ? 1 : 0 })} onDay={(x) => K.tick(k.id, x.key)} />
      ...
      {pick && <window.OverlayPortal><KeepMonthSheet lang={lang} current={k.masa} onClose={() => setPick(false)}
        onPick={(idx) => { K.setMasa(k.id, idx); setPick(false); }} /></window.OverlayPortal>}
    </article>
  );
}

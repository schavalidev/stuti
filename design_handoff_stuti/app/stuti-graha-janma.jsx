...
function GrahaJanmaTable({ lang = "deva", form = "all" }) {
  const G = window.STUTI_GRAHA_JANMA, V = window.STUTI_VRATA;
  if (!G) return null;
        ...
        <div className="gj-rows">
          {rows.map((r) => (
            <div className="gj-row" key={r.id}>
              {window.KeepBell && <span className="pit-row-bell"><window.KeepBell kind="vrata" id={G.vratId(r)} lang={lang} size={18} /></span>}
...
Object.assign(window, { GrahaJanmaTable });

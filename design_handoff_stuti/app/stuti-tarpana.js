...
window.STUTI_TARPANA = (function () {
  ...
  function profile() {
    let p = {};
    try { p = window.STUTI_PREFS.get().ritual || {}; } catch (e) {}
  ...
  try { if (window.STUTI_LOC) window.STUTI_LOC.subscribe(forget); } catch (e) {}
  ...
  function why(id, date, loc) {
    ...
    try {
      ...
      if (id === "sankranti-pitru" && window.STUTI_KSHANA) {
        const k = window.STUTI_KSHANA.fromSankranti(date, L);
  ...
  function onDay(date, loc) {
    const V = window.STUTI_VRATA; if (!V) return null;
  ...
  function occasionsOn(date, loc) {
    const L = loc || place(), Pa = P(), V = window.STUTI_VRATA, out = [];
...
window.STUTI_TARPANA_EXTRA = window.STUTI_TARPANA;

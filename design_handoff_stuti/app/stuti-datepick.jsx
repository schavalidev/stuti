        ...
        <React.Fragment>
          ...
          <div className="hd-pop">
            <div className="cal-nav hd-nav">
              <button type="button" className="icon-btn" onClick={() => step(-1)} aria-label={L.a("aPrevMonth")}><window.Icon name="prev" size={18} /></button>
              <div className="hd-yearpick">
                <button type="button" className="cal-nav-title display hd-title" onClick={() => setYearOpen((o) => !o)} aria-expanded={yearOpen}>
                  {new Date(cursor.y, cursor.m, 1).toLocaleDateString(locale, { month: "long", year: "numeric" })} <window.Icon name="chev" size={14} />
                </button>
                {yearOpen && (
              ...
              <button type="button" className="icon-btn" onClick={() => step(1)} aria-label={L.a("aNextMonth")}><window.Icon name="next" size={18} /></button>
            </div>
            ...
            <div className="cal-grid">
              ...
              {days.map(({ d, date, phase, isFull, isNew, out }) => (
                <button type="button" key={d} disabled={out}
                  ...
                  <span className="cal-marks">{(isFull || isNew) && <window.MoonPhase phase={phase} size={12} />}</span>
                </button>
              ))}
            </div>
...
Object.assign(window, { HouseDate });

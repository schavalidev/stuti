...
function HomeSearchBar({ go, lang = "deva" }) {
  ...
  return (
    <div className="home-search">
      ...
      {window.STUTI_VOICE && window.STUTI_VOICE.supported && (
...
function useProgress() {
  const [rec, setRec] = React.useState(() => window.STUTI_PROGRESS.get());
  React.useEffect(() => window.STUTI_PROGRESS.subscribe(setRec), []);
  return rec;
}
...
function useSankalpa() { return window.useFlyleaf(); }

/* everything a presentation of today needs: limbs, timings, saṅkalpa text */
function usePanchangaVM(lang, sk, deity) {
  const L = lang || "deva";
  const PA = window.AKSHARA_PANCHANGA, SK_CONST = window.SK_CONST, TR = window.STUTI_TRANSLIT, I18 = window.STUTI_L;
  ...
  React.useEffect(() => window.STUTI_PREFS.subscribe(() => bumpPrefs((n) => n + 1)), []);
  ...
  React.useEffect(() => {
    const p = window.AKSHARA_PANCHANGA.forDay(new Date(), loc, { instant: true });
    const mins = [p.karanaEndMin, p.tithiEndMin, p.nakEndMin, p.yogaEndMin].filter((m) => m != null);
    if (!mins.length) return;
    const nowMin = (() => {
      const tz = window.AKSHARA_PANCHANGA.effTz(loc, new Date());
...
function ParvaCard({ go, lang }) {
  ...
  React.useEffect(() => window.STUTI_PREFS.subscribe(() => bump((n) => n + 1)), []);
  React.useEffect(() => window.STUTI_TITHIS ? window.STUTI_TITHIS.subscribe(() => bump((n) => n + 1)) : undefined, []);
  let hits = [];
...
function ParvaOne({ hit, go, lang }) {
  ...
  const rule = v.ruleBy ? v.ruleBy(V.southern()) : (window.masaSys && window.masaSys() === "purnimanta" && v.ruleP) ? v.ruleP : v.rule;
...
function PreviewPill({ go, lang }) {
  ...
  if (!k) return null;
  const d = window.STUTI_VRATA.today();
...
function HomeA({ go, lang, overlayEl }) {
  const L = lang || "deva";
  const t = k => window.STUTI_L.t(k, L);
  ...
  const explainMu = () => {
    ...
    const time = window.AKSHARA_PANCHANGA.fmtTime(m.end);
  ...
  const panchangaBellOn = !!((window.STUTI_PREFS.get().remind || {}).tithi);
  const togglePanchangaNotif = () => {
    setMuNoteOpen(false);
    const N = window.STUTI_NUDGE, PR = window.STUTI_PREFS;
  ...
  return (
    <div className="view home home-a scroll" style={deityStyle(deity)}>
      {(() => {
        const rv = (window.RTU_VIS || {})[vm.pa.ritu.iast];
        /* a transition month blends both seasons here too, as Calendar and Māsa do */
        const mName = window.masaShown ? window.masaShown(vm.pa) : vm.pa.masa;
      ...
      <window.SkyHeader lang={L} go={go} />

      <HomeSearchBar go={go} lang={L} />

      <window.HomePrepCard lang={L} />
      ...
      <section className="rhb-card">
        ...
        <div className="rhb-hero">
          <window.MoonPhase phase={vm.pa.phase} size={62} />
          <div>
        ...
        <React.Fragment>
          ...
          <div className={"rhb-sheet" + (open ? " on" : "")} style={deityStyle(deity)}>
            ...
            <div className="rhb-sheet-head">
              ...
              <button className="rhb-sheet-x" onClick={() => setOpen(false)} aria-label={window.STUTI_L.a("aClose")}>×</button>
            </div>
            <div className="rhb-sheet-body"><window.SankalpaSheet vm={vm} sk={sk} L={L} go={go} /></div>
          </div>
...
function DailyCard({ go, lang = "deva", from = "daily" }) {
  const S = window.STUTI, L = window.STUTI_L;
  const favs = window.useFavs();
  const [, forceWeek] = React.useState(0);
  React.useEffect(() => window.STUTI_FAVS_WEEK.subscribe(() => forceWeek(n => n + 1)), []);
  const today = new Date().getDay();
  const [viewDay, setViewDay] = React.useState(today);
  const ids = favs.list();
  const weekIds = window.STUTI_FAVS_WEEK.list(viewDay);
  const items = ids.map(id => S.hymnById(id)).filter(Boolean);
  const weekItems = weekIds.map(id => S.hymnById(id)).filter(Boolean);
  const todayWeekItems = window.STUTI_FAVS_WEEK.list(today).map(id => S.hymnById(id)).filter(Boolean);
  ...
  const reorderDrop = (targetId, isWeek) => {
    ...
    const store = isWeek ? window.STUTI_FAVS_WEEK : favs;
  ...
  const row = (h, isWeek) => {
    const d = S.deityById[h.deity];
    const parts = window.hymnParts ? window.hymnParts(h) : null;
    const custom = parts && window.STUTI_RECITE.isCustom(h.id);
    return (
  ...
  return (
    <section className="daily">
      <div className="daily-head">
        ...
        {queue.length > 0 && (
          <button className="daily-begin" onClick={() => { const h = queue[0]; window.STUTI_NITYA_Q = queue.length > 1 ? { ids: queue.map(x => x.id), idx: 0, from } : null; go("reader", { deity: h.deity, hymn: h.id, from }); }}>
...
Object.assign(window, { HomeA, todayInfo, DailyCard, DailyView, ContinueCard, HomeSearchBar });

...
window.STUTI_FILL = (function () {
  ...
  function fillSankalpa(v, opts) {
    const S = window.STUTI_SANKALPA; if (!S) return null;
    let segs;
  ...
  function fillBlanks(v, opts) {
    const S = window.STUTI_SANKALPA, PA = window.AKSHARA_PANCHANGA, TR = window.STUTI_TRANSLIT;
    const K = (window.STUTI_SK || window.SK_CONST || {});
    if (!S || !PA) return null;
    let pa, id, loc, samv;
    try {
      ...
      id = S.identity();
      samv = window.samvatsaraFor(new Date());
    } catch (e) { return null; }
    ...
    const DS = window.STUTI_DESA, sk = id.sk;
    let desa = null;
    if (DS) {
      try {
  ...
  function hymn(h, opts) {
    ...
    const verses = h.verses.map((v) => {
      ...
      if (spec.recipients && window.STUTI_PITRU) {
        const slot = window.STUTI_PITRU.slotByVerse[v.n];
        if (slot) {

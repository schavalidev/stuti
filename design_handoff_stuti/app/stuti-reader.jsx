...
function lineMainText(line, lang) {
  if (!line) return "";
  return lang === "telugu" ? window.STUTI_TRANSLIT.convert(line.deva, "telugu")
       : lang === "deva" ? line.deva
       : line.iast;
}
...
function NamesSheet({ hymn, lang, onClose }) {
  ...
  const N = (window.STUTI_NAMES || {})[hymn.namesKey];
  if (!N) return null;
          ...
          <div className="nm-row" key={n.n}>
            ...
            <div className="nm-body">
              <div className="nm-name" style={{ fontFamily: font }}>
                {lang === "telugu" ? window.STUTI_TRANSLIT.convert(n.deva, "telugu") : lang === "deva" ? n.deva : n.iast}
              </div>
...
function NamaluList({ hymn, lang }) {
  ...
  if (!N) return null;
  const line = n => {
    if (lang === "telugu") return `శ్రీ ${window.STUTI_TRANSLIT.convert(toDativeDeva(n.deva), "telugu")} నమః`;
...
function DeityView({ deity, go, lang = "deva", showFormCounts = true, defaultForm = "all", backView = "browse", retView }) {
  ...
  const openAt = (h, at) => {
    ...
    const ritualSet = window.STUTI_RITUAL ? window.STUTI_RITUAL(h) : null;
  ...
  const entry = (h) => {
    ...
    const sibs = (window.STUTI_LIB && window.STUTI_LIB.siblingsOf) ? window.STUTI_LIB.siblingsOf(h) : [];
    return (
        ...
        <div className={"gs-en" + (h.catalog ? " soon" : "")}>
          ...
          {!h.catalog && (
            <span className="gs-marks">
              <window.LearnButton id={h.id} size={20} go={go} from="deity" />
  ...
  return (
    <div className="view deity-view deity-index scroll" style={deityStyle(deity, { flex: 1 })}>
      <button className="icon-btn gs-back" onClick={() => go(backView, { from: retView })} aria-label={window.STUTI_L.a("aBackDeities")}>
        <Icon name="back" />
      </button>
      ...
      {deity.id === "navagraha" && window.GrahaJanmaTable && <div className="gs-ix"><window.GrahaJanmaTable lang={lang} form={form} /></div>}
...
function RdScriptSeg({ lang, setLang }) {
  return (
    <div className="scriptseg" role="group" aria-label={window.STUTI_L.t("readIn", lang)}>
      {RD_SCRIPTS.map(o => (
...
function SizePicker({ dec, inc, atMin, atMax }) {
  return (
    <div className="rd-size" role="group" aria-label={window.STUTI_L.a("aTextSize")}>
      <button onClick={dec} disabled={atMin} aria-label={window.STUTI_L.a("aSmaller")}>−</button>
      <button onClick={inc} disabled={atMax} aria-label={window.STUTI_L.a("aLarger")}>+</button>
    </div>
  );
}

/* ---------------- The story behind the hymn — pūrva pīṭhikā, in full ---------------- */function StorySheet({ hymn, lang, onClose, onNext }) {
  const L = window.STUTI_L, a = hymn.about;
  const tel = (
    <>
      {a.te.map((p, i) => (
      ...
      {onNext && (
        <div className="st-onward">
          <button className="st-onward-btn" onClick={onNext}
            ...
            {window.STUTI_L.t("toPurva", lang)}<Icon name="arrow" size={18} />
          </button>
        </div>
      )}
    </div>
...
function ReaderView({ hymn: rawHymn, deity, go, theme, toggleTheme, lang, setLang, backView = "deity", retView }) {
  ...
  useEffect(() => {
    if (!window.STUTI_PITRU) return;
    return window.STUTI_PITRU.subscribe(() => setRegTick((n) => n + 1));
  }, []);
  const [regOpen, setRegOpen] = useState(null);   // slot id, or "" for the whole register
  const hymn = useMemo(
    () => (window.STUTI_FILL ? window.STUTI_FILL.hymn(rawHymn, { lang }) : rawHymn),
  ...
  const [active, setActive] = useState(() => {   // resume where the reciter left off
    ...
    const recite = window.STUTI_RECITE && window.STUTI_RECITE.get(hymn.id);
    if (cfgParts && recite && !window.STUTI_RECITE.hasPurva(recite)) {
  ...
  const [occId, setOccId] = useState(() => (occText ? window.toTarpanaRead() : null));
  ...
  const occHide = React.useMemo(
    () => (occText && occId && occId !== "all" ? window.toTarpanaHidden(hymn, occId) : null),
    [occText, occId, hymn.id]
  );
  const pickOcc = (id) => { setOccId(id); window.toTarpanaWrite(id); setOccAsk(false); };

  // Flatten every verse into a single sequence of recitable lines.
  const ritualSet = window.STUTI_RITUAL ? window.STUTI_RITUAL(hymn) : null;
  ...
  useEffect(() => {
    ...
    const nh = window.STUTI.hymnById(nqNextId);
  ...
  useEffect(() => {
    if (active >= 1 && !threadRef.current) { threadRef.current = true; window.STUTI_THREAD.mark("r", hymn.id); }
  }, [active, hymn.id]);

  // record the last-read hymn (+ position) so the home can offer to resume it
  useEffect(() => {
    if (active < 0) return;
    window.STUTI_PROGRESS.set({
  ...
  const padaInfo = (() => {
    ...
    const shown = lang === "telugu" ? ((tdv && tdv[ln.li]) || window.STUTI_TRANSLIT.convert(ln.deva, "telugu"))
                : lang === "deva" ? ln.deva : ln.iast;
    const dws = window.STUTI_PADA.words(shown);
    const iws = window.STUTI_PADA.words(ln.iast);
    if (!dws.length) return null;
    const i = Math.max(0, Math.min(pada.wordIdx, dws.length - 1));
    return { display: window.STUTI_PADA.trimTok(dws[i]), iast: iws[i] || dws[i], wordIdx: i, count: dws.length, lineIdx: pada.lineIdx };
  })();
  ...
  const speedWord = window.STUTI_L.t(speed < 0.9 ? "spdSlow" : speed <= 1.1 ? "spdNormal" : speed <= 1.6 ? "spdBrisk" : "spdSwift", lang);
  ...
  const parts = (() => {
    ...
    const recite = p && window.STUTI_RECITE && window.STUTI_RECITE.get(hymn.id);
    if (!recite) return p;
    return p.filter(x => (x.key !== "purva" || window.STUTI_RECITE.hasPurva(recite)) && (x.key !== "uttara" || window.STUTI_RECITE.hasUttara(recite)));
  })();
  ...
  if (!hymn.verses || hymn.verses.length === 0) {
    return <window.AwaitingText hymn={hymn} deity={deity} go={go} lang={lang} theme={theme} toggleTheme={toggleTheme} backView={backView} retView={retView} />;
  }
  ...
  const panelBody = (
    <React.Fragment>
      ...
      <button className={"icon-btn" + (shareLeft <= 0 ? " is-locked" : "")} title={limTitle(shareLeft, "share")} aria-label={window.STUTI_L.t("share", lang)}
        onClick={() => { panelDismiss(); gated("share", () => setShareOpen(true))(); }}>
        <Icon name="share" size={20} />
        {shareLeft <= 0 && <span className="lock-dot"><Icon name="lock" size={9} /></span>}
      </button>
      <button className={"icon-btn" + (printLeft <= 0 ? " is-locked" : "")} title={limTitle(printLeft, "printIt")} aria-label={window.STUTI_L.t("printIt", lang)}
        onClick={() => { panelDismiss(); gated("print", () => window.printStotra(hymn, lang, showMeaning))(); }}>
        <Icon name="print" size={20} />
        {printLeft <= 0 && <span className="lock-dot"><Icon name="lock" size={9} /></span>}
      </button>
      <button className="icon-btn" onClick={toggleTheme} aria-label={window.STUTI_L.t("dayNight", lang)} title={window.STUTI_L.t("dayNight", lang)}>
  ...
  return (
    <div className={"view reader" + (follow.on ? " is-following" : "")} style={deityStyle(deity, { flex: 1, "--rd-scale": fontScale })}>
      ...
      {occText && occId && <window.TarpanaOccasionBar occId={occId} lang={lang} onOpen={() => setOccAsk(true)} />}
      {occText && occAsk && <window.TarpanaOccasionSheet occId={occId} lang={lang} onPick={pickOcc} onClose={() => (occId ? setOccAsk(false) : pickOcc(window.toTarpanaGuess(new Date())))} />}
      ...
      <div className="reader-layers tight" hidden={learnOpen && (playing || (recordOn && (!!rec.take || rec.recording)))}>
        {hymn.about && (
          <button className="icon-btn" onClick={() => setStoryOpen(true)} aria-label={window.STUTI_L.t("story", lang)} title={window.STUTI_L.t("story", lang)}>
            <Icon name="book" size={19} />
          </button>
        )}
        {!namavaliOnly && <div className="rd-view" role="tablist" aria-label={window.STUTI_L.t("readAs", lang)}>
          <button className={"rd-view-btn" + (!flow ? " on" : "")} role="tab" aria-selected={!flow}
            aria-label={window.STUTI_L.t("viewVerse", lang)} title={window.STUTI_L.t("viewVerse", lang)}
            onClick={() => toRecite(flow ? flowAt.current : null)}>
            <span>{window.STUTI_L.t("viewVerse", lang)}</span>
          </button>
          <button className={"rd-view-btn" + (flow ? " on" : "")} role="tab" aria-selected={flow}
            aria-label={window.STUTI_L.t("viewFlow", lang)} title={window.STUTI_L.t("viewFlow", lang)}
            onClick={toFlow}>
            <span>{window.STUTI_L.t("viewFlow", lang)}</span>
          </button>
        </div>}
        <button className={"chip" + (showMeaning ? " chip-on" : "")} onClick={() => setShowMeaning(v => !v)}>
          {window.STUTI_L.t("meaning", lang)}
        </button>
      ...
      {hasRitual && !parts && (
        <div className="reader-layers tight rd-nyasa" hidden={learnOpen && (playing || (recordOn && (!!rec.take || rec.recording)))}>
          <button className={"chip" + (ritualOn ? " chip-on" : "")} onClick={toggleRitual}
            title={window.STUTI_L.t("ritualNote", lang)}
            style={lang === "telugu" ? { fontFamily: "var(--font-telugu)" } : lang === "deva" ? { fontFamily: "var(--font-deva)" } : null}>
            {window.STUTI_L.t("ritualChip", lang)}
          </button>
          <span className="rd-nyasa-note">{window.STUTI_L.t("ritualNote", lang)}</span>
        </div>
      )}

      {parts && (
        <div className="rd-parts" role="tablist" aria-label={window.STUTI_L.a("aParts")}>
          {parts.map((p, i) => {
            ...
            const label = window.STUTI_L.t(p.key, lang);
              ...
              return (
                <div key={p.key} className="rd-part-seg" role="presentation">
                  <button className={"rd-seg-opt" + (on && !namaluOpen ? " on" : "")} role="tab" aria-selected={on && !namaluOpen}
                    onClick={() => { setNamaluOpen(false); goVerse(p.at); }} aria-label={window.STUTI_L.t("tabNamavali", lang)} title={window.STUTI_L.t("tabNamavali", lang)}
                    style={lang === "telugu" ? { fontFamily: "var(--font-telugu)" } : null}>
                    {window.STUTI_L.t("tabNamavali", lang)}
                  </button>
                  <button className={"rd-seg-opt" + (namaluOpen ? " on" : "")} role="tab" aria-selected={namaluOpen}
                    onClick={() => setNamaluOpen(true)} aria-label={window.STUTI_L.t("tabNamalu", lang)} title={window.STUTI_L.t("tabNamalu", lang)}
                    style={lang === "telugu" ? { fontFamily: "var(--font-telugu)" } : null}>
                    {window.STUTI_L.t("tabNamalu", lang)}
                  </button>
                </div>
              );
            }
            return (
              <button key={p.key} className={"rd-part" + (on ? " on" : "")} role="tab" aria-selected={on}
                ...
                style={lang === "telugu" ? { fontFamily: "var(--font-telugu)" } : null}>
                {partIcon ? <Icon name={partIcon} size={18} /> : (p.key === "stotram" ? window.STUTI_L.t("gsNamavali", lang) : label)}
              </button>
            );
          })}
          ...
          {hymn.namesKey && (
            <button className="icon-btn rd-parts-end" onClick={() => setNamesOpen(true)} aria-label={window.STUTI_L.t("namesLbl", lang)} title={window.STUTI_L.t("namesLbl", lang)}>
          ...
          <div className="rd-seg">
            ...
            {learnMode === "repeat" && (
              <button className="rd-seg-rec rd-seg-repcount" onClick={cycleRepeatCount}
                aria-label={window.STUTI_L.a("aRepeatCount")} title={window.STUTI_L.a("aRepeatCount")}>
            ...
            <window.LearnButton id={hymn.id} size={20} go={go} from="reader" />
            ...
            <button className="rd-seg-exit" onClick={() => { setLearnOpen(false); setLearnMode(hasAudio ? "listen" : "repeat"); setPlaying(false); }}
              aria-label={window.STUTI_L.t("close", lang)} title={window.STUTI_L.t("close", lang)}>
              <Icon name="close" size={17} />
            </button>
          </div>
          {learnMode === "repeat" && micNote && (
            <div className="rd-peekhint">{window.STUTI_L.t("micDenied", lang)}</div>
          )}
          {learnMode === "memorize" && (
            <div className="rd-hints">
              {[["firstword", "hintFirstWord"], ["initials", "hintInitials"], ["blank", "hintBlank"]].map(([k, key]) => (
                <button key={k} className={"rd-hint-btn" + (hint === k ? " on" : "")}
                  onClick={() => { setHint(k); setPeek(false); }}>{window.STUTI_L.t(key, lang)}</button>
              ))}
            </div>
          )}
          {learnMode === "memorize" && !peek && (
            <div className="rd-peekhint">{window.STUTI_L.t("tapPeek", lang)}</div>
          )}
        </div>
      )}
            ...
            return (
              <div className="verse verse-active">
                ...
                {showMeaning && !masked && window.STUTI_MEAN(v, lang) && (
                  <div className="verse-meaning" style={{ fontFamily: window.STUTI_MEAN.font(v, lang) }}>
                ...
                {showMeaning && !masked && !window.STUTI_MEAN(v, lang) && hymn.namesKey && (
                  <button className="verse-mean-names" onClick={() => setNamesOpen(true)}>
                    <Icon name="lotus" size={14} /> {window.STUTI_L.t("meaningInNames", lang)}
                  </button>
                )}
                </div>
                {hasRitual && ritualSet.has(curVerse) && !masked && (
                  <button className="verse-skip" onClick={skipRitual}>
                    <Icon name="next" size={14} /> {window.STUTI_L.t("ritualSkip", lang)}
                  </button>
                )}
                ...
                {v.fill && !masked && (
                  <button className={"verse-fill" + (v.fill.kind === "blank" ? " is-blank" : "")}
                    ...
                    {v.fill.kind === "blank" ? window.rgT("stillBlank", lang)
                      : v.fill.kind === "sankalpa" ? window.STUTI_L.t("flTitle", lang)
                      : window.rgT("fromReg", lang)}
                  </button>
                )}
                {padaHint && !masked && (
                  <div className="pd-hint"><Icon name="lotus" size={15} /> {window.STUTI_L.t("tapWordHint", lang)}</div>
                )}
                ...
                {curVerse === hymn.verses.length - 1 && (
                  <div className="reader-end">
                    <Flame size={28} />
                    <div style={{ fontFamily: scriptFontFor(lang), fontSize: 20, color: "var(--accent-ink)" }}>{window.STUTI_L.t("shubham", lang)}</div>
                  </div>
                )}
              </div>
            );
      ...
      {repeatOn && playing ? (
        <div className={"rd-speak" + (speaking ? " on" : "")}>
          ...
          <span className="rd-speak-cap">{recordOn && rec.state === "recording" ? window.STUTI_L.t("recording", lang) : speaking ? window.STUTI_L.t("yourTurn", lang) + (repeatCount > 1 ? ` (${repIter}/${repeatCount})` : "") : window.STUTI_L.t(hasAudio ? "listenNow" : "followLine", lang)}</span>
          {speaking && <button className="rd-said" onClick={() => setActive(a => Math.min(lines.length - 1, a + 1))}>{window.STUTI_L.t("saidIt", lang)}</button>}
        </div>
      ) : null}
      ...
      {padaInfo && (
        <window.OverlayPortal>
          <window.PadaSheet pada={padaInfo} hymn={hymn} lang={lang} onClose={() => setPada(null)} onJump={jumpToOcc}
      ...
      {nextUp && (
        <div className="nextup-scrim" role="dialog" aria-modal="true">
          <div className="nextup-card" style={deityStyle(window.STUTI.deityById[nextUp.deity])}>
            <div className="nextup-k">{window.STUTI_L.t("startingNext", lang)}</div>
            <div className="nextup-title display" style={{ fontFamily: window.STUTI_L.font(lang) }}>{window.STUTI_L.hymnTitle(nextUp, lang)}</div>
            <div className="nextup-count" key={nqCount}>{Math.max(1, nqCount)}</div>
            <div className="nextup-actions">
              <button className="nextup-btn" onClick={() => setNqPaused(p => !p)} aria-label={window.STUTI_L.a(nqPaused ? "aPlay" : "aPause")}><Icon name={nqPaused ? "play" : "pause"} size={24} /></button>
              <button className="nextup-btn nextup-x" onClick={() => { setNextUp(null); window.STUTI_NITYA_Q = null; }} aria-label={window.STUTI_L.a("aClose")}>×</button>
            </div>
          </div>
        </div>
      )}
      {regOpen && <window.RgSlotSheet slot={regOpen} lang={lang} onClose={() => setRegOpen(null)}
      ...
      {panelOpen && !panelPin && (
        <window.OverlayPortal>
          <div className="rd-toppanel-scrim" onClick={() => setPanelOpen(false)}>
            <div className="rd-toppanel is-drop" role="dialog" aria-label={window.STUTI_L.t("settings", lang)} onClick={e => e.stopPropagation()}>{panelBody}</div>
          </div>
        </window.OverlayPortal>
      )}
      {shareOpen && (
        <window.OverlayPortal>
          <window.ShareSheet hymn={hymn} verse={hymn.verses[curVerse]} num={hymn.verses[curVerse] && hymn.verses[curVerse].n}
      ...
      <div className="reciter">
        <div className={"reciter-progress" + (lineScrub.drag ? " scrubbing" : "")} ref={lineScrub.ref} {...lineScrub.handlers}
          role="slider" aria-label={window.STUTI_L.a("aPosition")} aria-valuemin={1} aria-valuemax={lines.length} aria-valuenow={Math.max(1, active + 1)}
        ...
        <div className="reciter-row">
          <button className={"reciter-aux" + (loopMode !== "off" ? " on" : "")} onClick={cycleLoop} aria-label={window.STUTI_L.a("aLoop")}>
            <Icon name="repeat" size={22} />
            <span>{loopMode === "all" ? window.STUTI_L.t("loopAll", lang) : loopMode === "verse" ? window.STUTI_L.t("loopVerse", lang) : window.STUTI_L.t("loop", lang)}</span>
          </button>

          <div className="reciter-center">
            <button className={"reciter-step" + (scrollOn ? " is-verse" : "")} onClick={() => step(-1)} aria-label={window.STUTI_L.a(scrollOn ? "aPrevVerse" : "aPrevLine")}><Icon name="prev" size={24} /></button>
            <button className="reciter-play" onClick={togglePlay} aria-label={window.STUTI_L.a(playing ? "aPause" : "aPlay")}>
              <span className={"flamehalo" + (playing ? " on" : "")}><Flame size={30} lit={playing} /></span>
              <Icon name={playing ? "pause" : "play"} size={30} />
            </button>
            <button className={"reciter-step" + (scrollOn ? " is-verse" : "")} onClick={() => step(1)} aria-label={window.STUTI_L.a(scrollOn ? "aNextVerse" : "aNextLine")}><Icon name="next" size={24} /></button>
          </div>

          <button type="button" className={"reciter-speed" + (speedOpen ? " on" : "")} onClick={() => setSpeedOpen(o => !o)}
            aria-label={window.STUTI_L.a("aSpeed")} aria-expanded={speedOpen}>
            <span>{showSpeed(speed)}×</span>
          </button>
        </div>
        {speedOpen && (
          <div className="spd-strip" role="group" aria-label={window.STUTI_L.a("aSpeed")}>
              <div className="spd-row">
                ...
                <div className={"spd-track" + (speedScrub.drag ? " scrubbing" : "")} ref={speedScrub.ref} {...speedScrub.handlers}
                  role="slider" tabIndex={0} aria-label={window.STUTI_L.a("aSpeed")}
...
Object.assign(window, { DeityView, ReaderView, WordRun, RecMasked, hymnParts });

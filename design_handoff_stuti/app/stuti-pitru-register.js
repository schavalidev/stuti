...
window.STUTI_PITRU = (function () {
  ...
  const ownGotra = () => { try { return (window.STUTI_FLYLEAF.get().gotra || "").trim(); } catch (e) { return ""; } };
  ...
  function gotraPair(g) {
    g = (g || "").trim();
    if (!g) return null;
    const list = (window.STUTI_SK && window.STUTI_SK.GOTRAS) || (window.SK_CONST && window.SK_CONST.GOTRAS) || [];
  ...
  function remove(id) {
    ...
    if (e && e.tithiId) { try { window.STUTI_TITHIS.remove(e.tithiId); } catch (err) {} }
  ...
  function linkTithi(id, loc) {
    ...
    const T = window.STUTI_TITHIS; if (!T) return null;

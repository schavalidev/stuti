...
window.STUTI_KEEP = (function () {
  ...
  const modeFor = (kind, ref) => kind === "vrata" ? "vrata" : nomuCadence(window.STUTI_NOMU && window.STUTI_NOMU.get(ref)) === "daily" ? "daily" : "month";
  ...
  function markKept(id, on) {
    ...
    if (k.kept) { try { window.STUTI_THREAD.mark("p", "keep:" + id + ":" + k.keptOn); } catch (e) {} }
    save();
  }
  ...
  function keepVrataDay(id, date) { fresh(); const k = byId(id); if (!k) return; k.keptOn = dkey(date); try { window.STUTI_THREAD.mark("p", "keep:" + id + ":" + k.keptOn); } catch (e) {} save(); }

  const engines = () => {
    const PA = window.AKSHARA_PANCHANGA, LOC = window.STUTI_LOC; let place = null;
    ...
    return { panchanga: PA, place, vrata: window.STUTI_VRATA };
  };

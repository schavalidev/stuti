...
function usePrefs() {
  const [p, setP] = useStateN(() => window.STUTI_PREFS.get());
  useEffectN(() => window.STUTI_PREFS.subscribe(setP), []);
  return p;
}
...
function obsName(o, lang) {
  if (!o) return "";
  if (lang === "telugu") return window.STUTI_TRANSLIT.convert(o.deva || o.name, "telugu");
...
function nextNudge(remind, loc) {
  ...
  let mark = null;
  try {
    const pa = window.AKSHARA_PANCHANGA.forDay(at, loc);
...
function upcomingMarks(loc, n) {
  ...
  try {
    ...
    for (let i = 0; i < 40 && out.length < (n || 3); i++) {
      ...
      const pa = window.AKSHARA_PANCHANGA.forDay(day, loc);
...
function NudgePreview({ lang }) {
  const L = window.STUTI_L;
  const h = nudgeHymn();
  const p = window.STUTI_PREFS.get();
  return (
...
function SandhyaCues({ lang, loc, onNeedPermission }) {
  ...
  const toggle = (id) => {
    const on = !sy[id];
    window.STUTI_PREFS.setSandhya(id, on);
      ...
      {anyOn && (
        <React.Fragment>
          ...
          <div className="rm-times">
            {LEADS.map((n) => (
              <button key={n} className={"rm-time" + ((r.lead || 0) === n ? " on" : "")} onClick={() => window.STUTI_PREFS.setRemind({ lead: n })}>
...
function RemindSheet({ lang = "deva", onClose }) {
  ...
  const set = (patch) => window.STUTI_PREFS.setRemind(patch);

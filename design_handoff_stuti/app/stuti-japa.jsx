...
function ThreadCard({ lang = "deva" }) {
  ...
  useEffectJ(() => window.STUTI_THREAD.subscribe(() => force(x => x + 1)), []);
  const s = window.STUTI_THREAD.streak();
  const cells = window.STUTI_THREAD.last(28);
  const FlameI = window.Flame;
...
function ThreadStrip({ lang = "deva" }) {
  const L = window.STUTI_L, S = window.STUTI, T = window.STUTI_THREAD;
  const [, force] = useStateJ(0);
  ...
  const keptRows = (key) => {
    const r = T.day(key);
    if (!r) return [];
    const LIB = window.STUTI_LIB, rows = [];
    (r.r || []).forEach((id) => {
    ...
    (r.p || []).forEach((id) => {
      ...
      if (pr) rows.push({ k: L.t("threadKindPractised", lang), v: window.pick3 ? window.pick3(pr.name, lang) : pr.name.roman });
    });
        ...
        <Portal>
          ...
          <div className="tday-sheet" role="dialog" aria-label={dayLabelLong(sel)}>
            ...
            <div className="tday-head">
              ...
              <button className="tday-x" onClick={() => setSel(null)} aria-label={window.STUTI_L.a("aClose")}>×</button>
            </div>
...
function JapaEntryCard({ go, lang = "deva" }) {
  const L = window.STUTI_L;
  const [, force] = useStateJ(0);
  useEffectJ(() => window.STUTI_JAPA.subscribe(() => force(x => x + 1)), []);
  const today = JAPA_THREADS.reduce((s, m) => s + window.STUTI_JAPA.state(m.id).today, 0);
  return (
...
function MalaEntry({ mid, lang, onDone }) {
  const L = window.STUTI_L, J = window.STUTI_JAPA, T = window.STUTI_THREAD;
    ...
    <div className="mala-entry">
      ...
      <div className="mala-entry-row">
        <div className="mala-step">
          <button onClick={() => setRounds((n) => Math.max(1, n - 1))} aria-label={window.STUTI_L.a("aFewer")} disabled={rounds <= 1}>−</button>
          <span className="mala-step-n">{rounds}<i>{L.t(rounds === 1 ? "malaRoundL" : "malaRoundsL", lang)}</i></span>
          <button onClick={() => setRounds((n) => Math.min(999, n + 1))} aria-label={window.STUTI_L.a("aMore")}>+</button>
        </div>
      ...
      {open && <>
        ...
        <div className={"daypick-pop" + (place === "down" ? " daypick-pop-down" : "")} role="dialog" aria-label={label}>
          <div className="daypick-head">
            <button type="button" className="icon-btn" onClick={() => setView(new Date(view.getFullYear(), view.getMonth() - 1, 1))} aria-label={window.STUTI_L.a("aPrevMonth")}><span style={{ display: "inline-flex", transform: "rotate(90deg)" }}><Icon name="chev" size={16} /></span></button>
            <span className="daypick-month">
              <select value={view.getMonth()} onChange={(e) => setView(new Date(view.getFullYear(), +e.target.value, 1))} aria-label={window.STUTI_L.a("aPrevMonth")}>
            ...
            <button type="button" className="icon-btn" disabled={!nextOk} onClick={() => setView(new Date(view.getFullYear(), view.getMonth() + 1, 1))} aria-label={window.STUTI_L.a("aNextMonth")}><span style={{ display: "inline-flex", transform: "rotate(-90deg)" }}><Icon name="chev" size={16} /></span></button>
            <button type="button" className="icon-btn daypick-x" onClick={() => setOpen(false)} aria-label={window.STUTI_L.t("close", lang)}><Icon name="close" size={15} /></button>
          </div>
          ...
          <div className="daypick-foot"><button type="button" onClick={() => pick(maxD || todayD)}>{window.STUTI_L.t("today", lang)}</button></div>
        </div>
      </>}
    </span>
  );
...
function JapaView({ go, lang = "deva", embedded = false }) {
  ...
  const [mid, setMid] = useStateJ(() => {
    const last = window.STUTI_JAPA.lastId();
  ...
  const [st, setSt] = useStateJ(() => window.STUTI_JAPA.state(m.id));
  const [flash, setFlash] = useStateJ(false);
  const flashT = useRefJ(null);
  useEffectJ(() => { setSt(window.STUTI_JAPA.state(m.id)); window.STUTI_JAPA.setLast(m.id); }, [m.id]);
  useEffectJ(() => () => clearTimeout(flashT.current), []);
  const pos = st.today === 0 ? 0 : ((st.today - 1) % 108) + 1;
  const count = () => {
    const tn = window.STUTI_JAPA.bump(m.id);
    setSt(window.STUTI_JAPA.state(m.id));
    const done = tn % 108 === 0;
  ...
  const undo = () => { window.STUTI_JAPA.undo(m.id); setSt(window.STUTI_JAPA.state(m.id)); };
  const dots = [];
  for (let i = 0; i < 108; i++) {
  ...
  const monthDays = (() => { try { const n = new Date(); return window.STUTI_LEDGER.month(n.getFullYear(), n.getMonth()).japa.days; } catch (e) { return 0; } })();

...
function PrepSheet({ occ, lang, onClose }) {
  const L = window.STUTI_L, P = window.STUTI_PREP;
  const v = occ.v;
  const its = P.items(v);
  ...
  const printPack = () => window.printPrepPack(v, occ.date, lang, its, done);
  const row = (it) => {
            ...
            <React.Fragment>
              ...
              <div className="prep-copy-row prep-opts">
                ...
                <button className={"prep-copy" + (shareLeft <= 0 ? " is-locked" : "")} onClick={gated("share", share)}
                  ...
                  <Icon name="share" size={16} />{L.t("prepShare", lang)}{shareLeft <= 0 && <window.LimitLock size={12} />}
                </button>
                <button className={"prep-copy" + (packLeft <= 0 ? " is-locked" : "")} onClick={gated("pack", printPack)}
                  ...
                  <Icon name="print" size={16} />{L.t("prepPrint", lang)}{packLeft <= 0 && <window.LimitLock size={12} />}
                </button>
              </div>
            </React.Fragment>
          ) : (
...
function HomePrepCard({ lang }) {
  ...
  usePrE(() => window.STUTI_PREFS.subscribe(() => force((n) => n + 1)), []);
  let list = [];
...
function PrepCues({ lang }) {
  const L = window.STUTI_L, P = window.STUTI_PREP, V = window.STUTI_VRATA;
  window.usePrefs();
  const on = P.masterOn();
...
Object.assign(window, { PrepSheet, PrepHideSheet, HomePrepCard, PrepCues, printPrepPack });

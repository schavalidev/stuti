...
function useRegister() {
  const [, tick] = useRgS(0);
  useRgE(() => window.STUTI_PITRU.subscribe(() => tick((n) => n + 1)), []);
  return window.STUTI_PITRU;
}

/* ---- one name ---- */
function RgEntry({ entry, slot, lang, onDone }) {
  const P = window.STUTI_PITRU, L = window.STUTI_L;
  const { loc } = window.useLoc ? window.useLoc() : { loc: null };
  ...
  const spellOut = (r) => {
    const TR = window.STUTI_TRANSLIT; if (!TR || !r) return "";
  ...
  const toScreen = (v) => {
    const TR = window.STUTI_TRANSLIT; if (!TR || !rgIndic(v)) return v;
    const d = TR.toDeva(v);
    return lang === "telugu" ? TR.convert(d, "telugu") : d;
  };
  ...
  return (
    <div className="rg-edit">
      ...
      <div className="sk-field">
        ...
        <window.GotraField value={e.gotra || ""} onChange={set("gotra")} lang={lang} />
      </div>
      <div className="sk-field">
        <label className="sk-label">{rgT("deathL", lang)}</label>
        <div className="rg-when">
          <window.HouseDate value={dateVal} lang={lang} ariaLabel={rgT("deathL", lang)}
            onChange={(v) => {
        ...
        return (
          <div className="rg-preview">
            ...
            <div className="rg-preview-line" style={{ fontFamily: rgFont(lang) }}>
              {lang === "telugu" ? window.STUTI_TRANSLIT.convert(preview.deva, "telugu") : shown}
            </div>
        ...
        <div className="sk-field">
          ...
          {scriptField ? (
            <input className="sk-input" value={e.declD || ""} style={{ fontFamily: rgFont(lang) }}
              placeholder={(preview && (lang === "telugu" ? window.STUTI_TRANSLIT.convert(preview.deva, "telugu") : preview.deva)) || ""} onChange={(ev) => set("declD")(ev.target.value)} />
          ) : (
          ...
          <div className="pd-body scroll">
            ...
            {onOpenAll && (
              <button className="rg-allnames" onClick={onOpenAll}>
                {rgT("openAll", lang)}<window.Icon name="chev" size={14} />
              </button>
            )}
            <div style={{ height: 20 }} />
          </div>
...
function PitruRegisterView({ go, lang = "deva", backView }) {
  ...
  const entryFor = (e) => {
    const c = P.clause(e);
    const txt = !c ? "" : lang === "telugu" ? window.STUTI_TRANSLIT.convert(c.deva, "telugu") : lang === "roman" ? c.iast : c.deva;
    return txt || (e.nr || e.nd || "");
  };
  return (
    <div className="view rg-view scroll">
      <div className="topbar">
        <button className="icon-btn" onClick={() => go(backView || "calendar")} aria-label={L.a("aBack")}><window.Icon name="back" /></button>
        ...
        return (
          <section className={"tt-card rg-card rg-lcard" + (isOpen(ln.id) ? "" : " is-folded")} key={ln.id}>
            <button className="rg-lhead" onClick={() => toggle(ln.id)} aria-expanded={isOpen(ln.id)}>
              ...
              <window.Icon name="chev" size={16} />
            </button>
            {isOpen(ln.id) && <>
            {ln.gotra === "seed" && (
              <div className="sk-field rg-seed">
                <label className="sk-label">{rgT(ln.id === "matr" ? "seedMatr" : "seedSva", lang)}</label>
                <window.GotraField value={seeds[ln.id] || ""} onChange={(v) => P.setSeed(ln.id, v)} lang={lang} />
              </div>
            )}
        ...
        <window.OverlayPortal>
          <div className="pd-wrap">
            ...
            <div className="pd-sheet rg-sheet" role="dialog" aria-label={rgT("add", lang)}>
              <div className="pd-grip" />
              <button className="pd-x" onClick={() => setEditing(null)} aria-label={L.t("close", lang)}><window.Icon name="close" size={18} /></button>
              <div className="pd-body scroll">
...
Object.assign(window, { RgSlotSheet, PitruRegisterView, PitruRegisterDoor, useRegister, rgT, rgCount, rgFont });

...
function SettingsView({ go, lang, setLang, uiLang, setUiLang, theme, toggleTheme, openRemind, backView }) {
  ...
  const south = window.STUTI_VRATA && window.STUTI_VRATA.southern ? window.STUTI_VRATA.southern() : true;
  const tradOptions = [
  ...
  return (
    <div className="view libhub scroll">
      <div className="topbar">
        <button className="icon-btn" onClick={() => go(backView || "home")} aria-label={window.STUTI_L.a("aBack")}><Icon name="back" /></button>
        <div className="topbar-title display">{L.t("settings", uiLang)}</div>
        <span style={{ width: 44 }} />
      </div>

      {danaOpen && <window.DanaSheet lang={lang} onClose={() => setDanaOpen(false)} />}
      {fbOpen && <window.FeedbackSheet lang={lang} onClose={() => setFbOpen(false)} />}
        ...
        <SetChapter icon="locate" title={L.t("setChPlace", uiLang)}>
          <SetPanel>
            <SetBlock note={L.t("placeNote", uiLang)}>
              ...
              <div className="set-loc"><window.LocationControl /></div>
            </SetBlock>
        ...
        <SetChapter icon="spark" title={L.t("setChHelp", uiLang)}>
          <SetPanel>
            ...
            <SetBlock>
              ...
              <p className="set-about">{L.t("aboutProof", uiLang).replace("{p}", window.STUTI_PROOF.counts().proofed).replace("{a}", window.STUTI_PROOF.counts().awaiting)}</p>
              <p className="set-about set-support">{L.t("supportLine", uiLang).replace("{days}", window.STUTI_BUILD.REPLY_DAYS)} <a href={"mailto:" + window.STUTI_BUILD.SUPPORT}>{window.STUTI_BUILD.SUPPORT}</a></p>
              <window.BuildStamp lang={lang} noReset />
            </SetBlock>
          </SetPanel>
        </SetChapter>
...
function NotifPermRow({ lang: uiLang }) {
  const L = window.STUTI_L, N = window.STUTI_NUDGE;
...
Object.assign(window, { SettingsView, SetSection, SetRow, SetChoice, SetChapter, SetPanel, SetField, SetBlock, CarrySection, NotifPermRow, FlyleafBlock });

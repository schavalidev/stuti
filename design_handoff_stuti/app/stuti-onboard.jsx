...
function ObPlace({ lang }) {
  const L = window.STUTI_L;
  const { locId, detected, geo, loc, LOCS, setLocId, detect } = window.useLoc();
...
function Onboarding({ lang, setLang, onDone }) {
  const S = window.STUTI, L = window.STUTI_L, P = window.STUTI_PREFS;
        ...
        <div className="ob-scroll scroll">
          ...
          {i === 4 && (
            <ObStep lang={lang} title={L.t("obSankalpa", lang)} sub={L.t("obSankalpaSub", lang)}>
              ...
              <window.KnownClauses lang={lang} sk={sk} />
              <window.FlyleafForm sk={sk} lang={lang} />

...
const maFont = (lang) => window.STUTI_L.font(lang);
...
function maProgress(idx) {
  const MA = window.STUTI_MASA, r = MA.rangeOf ? MA.rangeOf(idx) : null;
  if (!r) return null;
...
function MasaLens({ lang, onOpen }) {
  ...
  return (
    <div className="lens-pad">
      ...
      <button className="masa-now" style={MASA_MIX[hero.id] ? { "--rtu": rtu.hue, "--rtu-b": MASA_MIX[hero.id].hueB, "--i": 0 } : { "--rtu": rtu.hue, "--i": 0 }} data-mix={MASA_MIX[hero.id] ? "1" : undefined} onPointerDown={window.tileDown} onPointerLeave={window.tileRest} onPointerUp={window.tileRest} onPointerCancel={window.tileRest} onClick={() => onOpen(hero.id)}>
        <window.SeasonAmbient kind={rtu.kind} dense mix={MASA_MIX[hero.id] && MASA_MIX[hero.id].kinds} heat={MASA_MIX[hero.id] && MASA_MIX[hero.id].heat} sunHue={MASA_MIX[hero.id] && MASA_MIX[hero.id].sunHue} sunBright={MASA_MIX[hero.id] && MASA_MIX[hero.id].sunBright} />
        <span className="niche-sheen" aria-hidden="true"></span>
        <span className="masa-now-top">
          <span className="masa-now-rtu">
            <window.RtuGlyph kind={rtu.kind} size={17} />
      ...
      <div className="tile-grid">
        {rest.map((m, n) => {
          ...
          return (
            <button key={m.id} className="masa-tile masa-tile-plain" onPointerDown={window.tileDown} onPointerLeave={window.tileRest} onPointerUp={window.tileRest} onPointerCancel={window.tileRest} style={mix ? { "--rtu": r.hue, "--rtu-b": mix.hueB, "--i": n, animationDelay: `${60 + n * 30}ms` } : { "--rtu": r.hue, "--i": n, animationDelay: `${60 + n * 30}ms` }} data-mix={mix ? "1" : undefined} onClick={() => onOpen(m.id)}>
              <span className="niche-sheen" aria-hidden="true"></span>
...
function MasaDetail({ masaId, go, lang, onBack, onOpen }) {
  const S = window.STUTI, L = window.STUTI_L, MA = window.STUTI_MASA, LIB = window.STUTI_LIB;
  ...
  return (
    <div className="view libhub scroll masa-detail-page" data-mix={MASA_MIX[m.id] ? "1" : undefined} style={MASA_MIX[m.id] ? { "--rtu": maRtu(m.id).hue, "--rtu-b": MASA_MIX[m.id].hueB } : { "--rtu": maRtu(m.id).hue }}>
      <window.SeasonAmbient kind={maRtu(m.id).kind} dense mix={MASA_MIX[m.id] && MASA_MIX[m.id].kinds} heat={MASA_MIX[m.id] && MASA_MIX[m.id].heat} sunHue={MASA_MIX[m.id] && MASA_MIX[m.id].sunHue} sunBright={MASA_MIX[m.id] && MASA_MIX[m.id].sunBright} />
        ...
        <header className="md-hero">
          <div className="md-meta">
            <div className="md-rtu">
              <window.RtuGlyph kind={rtu.kind} size={15} />
              <span style={{ fontFamily: font }}>{maP(rtu.name, lang)}</span>
...
Object.assign(window, { MasaLens, MasaDetail, masaMixFor });

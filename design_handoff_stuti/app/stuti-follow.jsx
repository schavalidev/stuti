/* ============================================================
   Carried from the app's code: stuti-app/src/stuti-follow.tsx
...
const vocabIndex = {};
const grammars = {};
async function grammarForLines(voskLang, key, lines) {
    if (grammars[key])
        return grammars[key];
    try {
        const t0 = Date.now();
        if (!vocabIndex[voskLang])
            vocabIndex[voskLang] = indexVocab(await voskVocab(voskLang));
        const t1 = Date.now();
        ...
        voskLog("no grammar: " + String(e && e.message || e));
...
function useFollow({ hymn, lines, lang, active, setActive, setWord, setPlaying, onDone }) {
    ...
    const log = React.useRef([]); // this session, for sharing: what was heard and where the light went
    const t0 = React.useRef(0);
    const grammar = React.useRef(null);
    ...
    const webCap = React.useRef(null);
    ...
    const note = (line, fresh = false) => {
        if (fresh)
            log.current = [line];
        else
            log.current.push(line);
        if (log.current.length > 4000)
            log.current.splice(0, 1000);
        voskNote(line, fresh);
    };
    const mark = (line) => { if (recT0.current && !entered.current.has(line))
        entered.current.set(line, (Date.now() - recT0.current) / 1000); };
    const apply = (pos) => {
        mark(pos.line);
    ...
    const wire = (r) => {
        ...
        r.onerror = (ev) => {
            ...
            if (/not-allowed|service-not-allowed|denied/.test(e))
                stop("denied");
            else if (/model-missing/.test(e))
                stop("needs-model");
            /* "no-speech" and friends just mean a quiet stretch */
        };
        r.onstart = () => { if (armRec.current && !recT0.current)
            beginCapture(); };
        r.onlevel = (v) => setLevel(v);
        r.onraw = (text) => { setRaw(text.slice(-60)); setEvents((n) => n + 1); note(((Date.now() - t0.current) / 1000).toFixed(2) + " raw " + text); };
        r.onend = () => {
            rec.current = null;
            /* the browser's recogniser closes after a pause: reopen it. Vosk only
               ends when we stop it, so this never loops there. */
            if (onRef.current && !native)
    ...
    const endCapture = (silent = false) => {
        ...
        const weights = lines.map((l) => (l.iast || l.deva || "").replace(/\s+/g, "").length);
        ...
        recT0.current = 0;
        armRec.current = false;
        clearInterval(ticker.current);
        setRecOn(false);
        const w = webCap.current;
        webCap.current = null;
        if (silent) {
            if (w)
                w.stop().catch(() => { });
        ...
        if (native)
            setKeep({ dur, cues, linesLit, blob: null });
        else if (w)
            w.stop().then(({ blob, dur: d }) => setKeep({ dur: d || dur, cues, linesLit, blob })).catch(() => setKeep({ dur, cues, linesLit, blob: null }));
        else
            setKeep({ dur, cues, linesLit, blob: null });
    };
    const record = () => {
    ...
    const doKeep = async () => {
        ...
        try {
            ...
            setKeep({ ...k, failed: true });
        }
    };
    const discardKeep = () => setKeep(null);
    const openShelf = () => setShelf(true);
    const closeShelf = () => setShelf(false);
    const refreshRecs = () => { if (hymn && hymn.id)
        listRecitations(hymn.id).then(setRecs).catch(() => { }); };
    const listen = () => {
        if (!onRef.current)
            return;
        let r;
        try {
            r = native ? new VoskRecognition() : new (BrowserSR())();
        }
        catch (e) {
            stop("unsupported");
            return;
    ...
    const begin = async () => {
        ...
        if (native && voskLang === "hi") {
            ...
            if (!onRef.current)
                return; // stopped while the grammar was being built
        }
        listen();
    };
    const share = () => {
        if (!native)
            return;
        if (onRef.current)
            stop(undefined, true); // close the session, then hand it over
    ...
    const start = async () => {
        ...
        if (native && !(await voskModelReady(voskLang))) {
    ...
    const download = async () => {
        ...
        setPct(0);
        try {
            await voskDownload(voskLang, setPct);
            begin();
        }
        catch (e) {
            setStatus("failed");
    ...
    React.useEffect(() => {
        if (on && eng.current && active !== applied.current && active >= 0) {
            eng.current.seek(active);
            applied.current = active;
        }
    }, [active, on]);
    /* a new script changes the recogniser language: restart the ears */
    React.useEffect(() => { if (on && !recT0.current) {
        stop();
        start();
...
function FollowButton({ follow, lang }) {
    if (!follow.supported)
        return null;
    ...
    const click = () => { if (held.current) {
        held.current = false;
        return;
    } follow.toggle(); };
    return (<button type="button" className={"icon-btn rd-follow-btn" + (follow.on ? " is-on" : "") + (follow.status === "lost" ? " is-lost" : "")} onClick={click} onPointerDown={down} onPointerUp={up} onPointerCancel={up} onPointerLeave={up} onContextMenu={(e) => { e.preventDefault(); }} aria-pressed={follow.on} aria-label={label} title={label}>
      <Icon name="mic" size={20}/>
    </button>);
}
function FollowChip({ follow, lang }) {
    ...
    if (st === "needs-model") {
        return (<div className="rd-follow-chip is-needs-model" role="dialog" aria-live="polite">
    ...
    if (follow.keep) {
        const k = follow.keep;
        return (<div className="rd-follow-chip is-needs-model is-keep" role="dialog" aria-live="polite">
    ...
    if (follow.kept && !follow.on) {
        return (<div className="rd-follow-chip is-done is-kept" role="status" aria-live="polite">
    ...
    if (!follow.on && follow.shelf)
        return <RecitationsSheet follow={follow} lang={lang}/>;
...
function RecordChip({ follow, lang }) {
    if (!follow.supported)
        return null;
    const on = follow.recOn;
    return (<button type="button" className={"chip chip-record" + (on ? " chip-on" : "")} onClick={on ? () => follow.stop() : follow.record} aria-pressed={on} title={t("recordTitle", lang)}>
      {on ? "■ " + t("stopRec", lang) : "● " + t("record", lang)}
    </button>);
}
/* ● beside the title: how many recitations of this text are kept, and the
   way to them */
function RecitationsButton({ follow, lang }) {
    if (!follow.recs.length)
        return null;
    const label = t("openShelf", lang, { n: follow.recs.length });
    return (<button type="button" className="icon-btn rd-rec-btn" onClick={follow.openShelf} aria-label={label} title={label}>
...
function RecitationsSheet({ follow, lang }) {
    ...
    const play = (r) => {
        ...
        el.src = recitationSrc(r);
        el.currentTime = 0;
        el.play().then(() => setPlaying(r.id)).catch(() => setPlaying(null));
    };
    React.useEffect(() => { const el = audio.current; if (!el)
        return; const done = () => setPlaying(null); el.addEventListener("ended", done); return () => { el.removeEventListener("ended", done); el.pause(); }; }, []);
    React.useEffect(() => { if (arm) {
        const id = setTimeout(() => setArm(null), 4000);
    ...
    return (<OverlayPortal>
      ...
      <div className="rd-rec-sheet" role="dialog" aria-label={t("shelf", lang)}>
        ...
        {follow.recs.map((r) => (<div key={r.id} className={"rd-rec-row" + (playing === r.id ? " is-playing" : "")}>
            ...
            <button type="button" className={"rd-follow-act" + (arm === r.id ? " danger" : "")} onClick={() => { if (arm === r.id) {
            setArm(null);
            if (playing === r.id && audio.current) {
                audio.current.pause();
                setPlaying(null);
            }
            deleteRecitation(r).catch(() => { });
        }
        else
            setArm(r.id); }}>
...
Object.assign(window, { useFollow, FollowButton, FollowChip, RecordChip, RecitationsButton, RecitationsSheet });
})();

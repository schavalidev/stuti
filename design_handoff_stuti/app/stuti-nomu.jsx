...
const nomFont = (lang) => window.STUTI_L.font(lang);

/* the classifier lives in STUTI_KEEP so the bell and these chips agree */
const NOMU_CADENCE = window.STUTI_KEEP.NOMU_CADENCE;
const nomuCadence = (n) => window.STUTI_KEEP.nomuCadence(n);

function NomuRow({ n, lang, onOpen, no }) {
  const S = window.STUTI, d = S.deityById[n.deity];
  const font = nomFont(lang);
  return (
    <div className="gs-en">
      <window.KeepBell kind="nomu" id={n.id} lang={lang} />
...
function NomuLens({ go, lang, onOpen }) {
  const L = window.STUTI_L, N = window.STUTI_NOMU;
...
function NomuDetail({ nomuId, go, lang, onBack }) {
  const S = window.STUTI, L = window.STUTI_L, N = window.STUTI_NOMU;
...
Object.assign(window, { NomuLens, NomuDetail });

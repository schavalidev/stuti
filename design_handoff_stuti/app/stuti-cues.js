...
window.STUTI_CUES = (function () {
  ...
  function obligations(ctx) {
    ...
    try {
      const TT = window.STUTI_TITHIS, V = ctx.engines.vrata;
  ...
  function ctxNow(now) {
    ...
    let prefs = {}; try { prefs = window.STUTI_PREFS.get(); } catch (e) {}
    let japa = {}, plans = {};
    try { japa = JSON.parse(localStorage.getItem("stuti-japa") || "{}") || {}; } catch (e) {}
    try { plans = JSON.parse(localStorage.getItem("stuti-plans") || "{}") || {}; } catch (e) {}
    let vows = []; try { vows = window.STUTI_VOWS.list(); } catch (e) {}
    let keeps = []; try { keeps = window.STUTI_KEEP.list(); } catch (e) {}
    let fired = {};
    ...
    return {
      ...
      state: {
        ...
        hymnById: (id) => { try { return window.STUTI.hymnById(id); } catch (e) { return null; } },
      },
      engines: {
        panchanga: PA, sandhya: window.STUTI_SANDHYA,
        plans: window.STUTI_PLANS,
        vows: window.STUTI_VOWS ? { occ: window.STUTI_VOWS.occ, term: window.STUTI_VOWS.term } : null,
        keep: window.STUTI_KEEP || null, vrata: window.STUTI_VRATA || null,
        ledger: window.STUTI_LEDGER || null,
      },
    };
  }

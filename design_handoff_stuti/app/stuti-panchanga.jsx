/* ============================================================
   ...
   Built on the astronomical engine (window.AKSHARA_PANCHANGA):
...
/* ------------------------------------------------------------
   ...
   ------------------------------------------------------------ */
function useLoc() {
  const LOC = window.STUTI_LOC;
  const LOCS = window.AKSHARA_PANCHANGA.locations;
  const [, force] = usePS(0);
...
const locFold = (s) => window.STUTI_TRANSLIT.fold(s);
...
function LocationControl({ compass }) {
  ...
  const pickPlace = (p) => { window.STUTI_LOC.pick(p); setQ(""); setOpen(false); };

  return (
    <div className={"locctl" + (open ? " open" : "")}>
      {compass && window.CompassDial && <window.CompassDial lang={(() => { try { return localStorage.getItem("stuti-lang") || "roman"; } catch (e) { return "roman"; } })()} />}
          ...
          <div className="loc-menu loc-menu-float" role="listbox">
            <div className="loc-head">
              <div className="loc-search">
                ...
                {q && <button className="loc-search-clear" onClick={() => setQ("")} aria-label={window.STUTI_L.a("aClear")}>×</button>}
              </div>
...
Object.assign(window, { MoonPhase, samvatsaraFor, useLoc, LocationControl });
...
Object.assign(window, {
  RtuGlyph, AyanaGlyph, SeasonAmbient, RTU_VIS, Snowflake,
  SK_CONST: { GOTRAS, KARMAS, VARA_GRAHA, YOGA_DEVA, KARANA_DEVA },
});

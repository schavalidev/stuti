...
function toGuess(date) {
  try {
    const on = window.STUTI_TARPANA.occasionsOn(date || new Date()) || [];
...
function TarpanaOccasionBar({ occId, lang, onOpen }) {
  ...
  return (
    <div className="rd-occ-bar">
      <span className="rd-occ-name" style={{ fontFamily: window.STUTI_L.font(lang) }}>
...
function TarpanaOccasionSheet({ occId, lang, onPick, onClose }) {
  ...
  const font = window.STUTI_L.font(lang);
  return (
...
Object.assign(window, { TarpanaOccasionBar, TarpanaOccasionSheet, toTarpanaGuess: toGuess, toTarpanaHidden: toHidden, toTarpanaRead: toRead, toTarpanaWrite: toWrite });

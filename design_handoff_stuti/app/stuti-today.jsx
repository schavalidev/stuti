...
function TodayBand({ go, lang = "deva" }) {
  const L = window.STUTI_L, C = window.STUTI_CUES;
  ...
  const hymnOf = (id) => { try { return window.STUTI.hymnById(id); } catch (e) { return null; } };
  const japaVowName = (dId, label) => { if (label) return label + " \u00b7 " + L.t("vowKindJapa", lang); try { const d = window.STUTI.deityById[dId]; return L.name(d, lang) + " \u00b7 " + L.t("vowKindJapa", lang); } catch (e) { return L.t("vowKindJapa", lang); } };
  const goJapa = (dId) => { try { window.STUTI_JAPA.setLast(dId); } catch (e) {} go("japa"); };
  const malaHas = (dId) => { try { return window.JAPA_THREADS.some(m => m.id === dId); } catch (e) { return false; } };
      ...
      <div className="tb-head">
        ...
        {tithi && (
          <span className="tb-mark" style={{ fontFamily: font }}>
            {lang === "telugu" ? window.STUTI_TRANSLIT.convert(tithi.obs.deva || tithi.obs.name, "telugu")
...
Object.assign(window, { TodayBand });

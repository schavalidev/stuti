...
function useLimitLeft(gate) {
  ...
  useLimE(() => window.STUTI_LIMITS.subscribe(() => bump()), []);
  return window.STUTI_LIMITS.left(gate);
}

function LimitLock({ size = 13 }) {
  return <window.Icon name="lock" size={size} />;
}

function LimitSheet({ lang = "roman", onClose }) {
  const L = window.STUTI_L, LM = window.STUTI_LIMITS;
        ...
        <button className="pd-x" onClick={onClose} aria-label={window.STUTI_L.t("close", lang)}><window.Icon name="close" size={18} /></button>
          ...
          <div className="pd-body scroll">
            <div className="dana-soon">
              <div className="dana-soon-mark"><window.Icon name="lock" size={24} /></div>
...
Object.assign(window, { useLimitLeft, LimitLock, LimitSheet });

...
function NityaView({ go, lang = "deva", showPractices = true, openRemind, initLens }) {
  ...
  const arr = window.useLensArrange({ lenses: NITYA_LENSES, storageKey: "stuti-nitya-lens-order", lang });
  ...
  window.useLensSwipe({ viewRef, trackRef, lenses: nityaLenses, lens, onChange: choose, disabled: arr.arranging });
  return (
    <div className="view libhub scroll" ref={viewRef}>
      ...
      <div className={"lens-pad" + (lens === "nomu" ? " lens-olive" : "")} data-lens={lens}>
        {lens === "patha" ? (
          <React.Fragment>
            <window.ReciteTracker lang={lang} />
            <window.DailyCard go={go} lang={lang} />
          </React.Fragment>
        ) : lens === "learn" ? (
          <React.Fragment>
            <window.LearnTracker lang={lang} />
            <window.PlanEntryCard go={go} lang={lang} />
          </React.Fragment>
        ) : lens === "japa" ? (
          <React.Fragment>
            <window.JapaTracker lang={lang} />
            <window.JapaView go={go} lang={lang} embedded={true} />
          </React.Fragment>
        ) : lens === "ledger" ? (
          <window.LedgerLens lang={lang} onLens={choose} go={go} />
        ) : lens === "nomu" ? (
          <window.NomuTracker go={go} lang={lang} />
        ) : (
          <React.Fragment>
            <window.TodayBand go={go} lang={lang} />
            {openRemind && <window.RemindCard lang={lang} onOpen={openRemind} />}
            <window.VowsCard go={go} lang={lang} />
            <window.WatchCard go={go} lang={lang} />
            {showPractices && (
              <React.Fragment>
...
function PracticesView({ go, lang = "deva" }) {
  ...
  return (
    <div className="view libhub scroll">
      <div className="topbar">
        <button className="icon-btn" onClick={() => go("browse")} aria-label={window.STUTI_L.a("aBack")}><Icon name="back" /></button>
...
function PracticeView({ practice, go, lang = "deva", backView = "daily" }) {
  const S = window.STUTI, L = window.STUTI_L, LIB = window.STUTI_LIB;
  const p = practice;
  const d = S.deityById[p.deity];
  const total = p.steps.length;
  ...
  useEffectPr(() => { if (allDone) window.STUTI_THREAD.mark("p", p.id); }, [allDone, p.id]);
  ...
  return (
    <div className="view practice-view scroll" style={deityStyle(d, { flex: 1 })}>
      ...
      <div className="lens-pad" data-lens={lens}>
...
Object.assign(window, { NityaView, PracticesView, PracticeView, PracticeCard, usePracticeDone, SandhyaNoteView });

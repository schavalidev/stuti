/* ============================================================
   Carried from the app's code: stuti-app/src/stuti-follow-engine.ts
...
function devaToLatin(s) {
    ...
    for (let i = 0; i < chars.length; i++) {
        ...
        if (ch === "ं" || ch === "ँ") {
            out += i + 1 < chars.length && /[\u0915-\u0939]/.test(chars[i + 1]) ? "n" : "m";
            continue;
        }
        if (DEVA[ch] !== undefined)
            out += DEVA[ch];
        else if (/[ऀ-ॿ]/.test(ch))
            continue; // stray marks, digits, etc.
        else
            out += ch;
    }
    return out;
...
class FollowEngine {
    constructor(lines) {
        this.toks = [];
        this.lineStart = []; // first token index of each line
        this.heard = []; // keys of words already final
        this.partial = []; // keys of the current utterance's words
        this.idx = -1; // token index of the current position (-1 = not yet)
        this.misses = 0;
        this.lastMoveAt = 0;
        this.now = 0; // the clock the caller keeps (real, or a replay's)
        this.lastMoveOn = ""; // the heard tail that last moved the light
        this.lastMoveKeys = 0; // ...and how many words it had
        ...
        lines.forEach((l, li) => {
            ...
            wordsOf(l.iast || l.deva).forEach((w, wi) => {
                const key = keyOf(w);
                if (key)
                    this.toks.push({ line: li, word: wi, key });
            });
        });
    }
    get pos() {
    ...
    get lead() {
        if (this.idx < -1 || !this.toks.length)
            return null;
    ...
    hear(transcript, isFinal, at = Date.now()) {
        this.now = at;
        ...
        if (isFinal) {
            this.heard = this.heard.concat(ks).slice(-8);
            this.partial = [];
        }
        return moved;
    }
    ...
    runEndingAt(end, len, heard = "") {
        ...
        if (heard && !/[mn]$/.test(heard) && /[mn]$/.test(s) && s.length > 2)
            s = s.slice(0, -1);
        for (let i = end - 1; i >= 0 && s.length < len; i--)
            s = this.toks[i].key + s;
    ...
    runStartingAt(start, len) {
        let s = "";
        for (let i = start; i < this.toks.length && s.length < len; i++)
            s += this.toks[i].key;
        return s.slice(0, len);
    }
    isLineStart(i) { return i === 0 || this.toks[i].line !== this.toks[i - 1].line; }
    ...
    best(from, to, keys, cur, strict, skipCost = 0.05) {
        const scores = [];
        const inside = [];
        ...
        for (let e = Math.max(0, from); e <= Math.min(to, this.toks.length - 1); e++) {
            ...
            for (let n = minN; n <= Math.min(6, keys.length); n++) {
                ...
                if (lone && h.length < (strict ? 6 : 4))
                    continue;
                if (strict && n > 1 && h.length < 8)
                    continue;
                ...
                if (!strict && n === 1 && h.length >= 4) {
                    ...
                    if (key.length >= h.length + 2) {
                        let bs = 0;
                        for (let p = 1; p + h.length < key.length; p++) {
                            const sc = sim(h, key.slice(p, p + h.length));
                            if (sc > bs)
                                bs = sc;
                        }
    ...
    align(fresh) {
        ...
        if (joined.length < 3)
            return null; // too little to go on
        ...
        if (near.idx >= 0 && near.idx < cur && this.status !== "lost")
            return null;
        /* the head or middle of the word the light is already on: stay */
        if (near.idx >= 0 && near.inside && near.idx <= cur)
            return null;
        if (near.idx >= 0)
            return this.moveTo(near.inside ? near.idx - 1 : near.idx, now, joined);
        ...
        if (head >= 0 && head !== cur + 1 && !tooSoon)
            return this.moveTo(head - 1, now, joined);
        if (fresh)
            this.misses++;
        if (this.misses >= 5)
            this.status = "lost"; // even after "done": starting over re-finds
...
Object.assign(window, { fineKey, keyOf, wordsOf, FollowEngine });
})();
